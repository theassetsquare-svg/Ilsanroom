...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!venue) return { title: { absolute: '요정을 찾을 수 없습니다 | 오늘밤어디' } };
  return {
    title: getHookingTitle(venue),
    description: getHookingDescription(venue),
    openGraph: { title: `${venue.nameKo} | 오늘밤어디`, description: venue.shortDescription, images: [{ url: getVenueOgImageBySlug(slug), width: 1200, height: 630 }] },
  };
}
...
const defaultFaqs = (name: string) => [
  ...
  { question: `영업시간은 어떻게 되나요?`, answer: `영업시간은 기본정보 탭에서 확인하세요. 방문 전 전화로 확인하시는 것을 권장합니다.` },
  ...
  return (
    <div className="rounded-2xl border border-neon-border/50 bg-neon-surface/30 p-8 space-y-6">
      ...
      <div className="space-y-4 text-sm leading-relaxed text-neon-text-muted">
        <p>
          명월관은 경기도 고양시 장항로 895-1에 자리한 격조 높은 한정식 문화 공간입니다.
