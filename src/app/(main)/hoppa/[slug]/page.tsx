...
export function generateStaticParams() {
  return getVenuesByCategory('hoppa').map((v) => ({
    slug: v.slug,
  }));
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!venue) return { title: '호빠를 찾을 수 없습니다 | NEON' };
  return {
    title: `${venue.nameKo} - ${venue.regionKo} 호빠 | NEON`,
    ...
    <div className="bg-neutral-950">
      ...
      <VenueJsonLd
        ...
        breadcrumbItems={[
          { name: 'NEON', url: '/' },
          { name: '호빠', url: '/hoppa' },
          { name: venue.nameKo, url: `/hoppa/${venue.slug}` },
        ]}
        reviews={[
      ...
      <section className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
        <Breadcrumb items={[{ label: '호빠', href: '/hoppa' }, { label: venue.nameKo }]} />
      </section>

      <section className="relative overflow-hidden border-b border-neutral-800">
        <div className="absolute inset-0 bg-gradient-to-b from-pink-950/30 to-neutral-950" />
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {venue.atmosphere.map((a) => <Badge key={a} variant="hoppa">{a}</Badge>)}
              </div>
            </div>
            <div>
          ...
          <div>
            <div className="rounded-2xl border border-neutral-800 bg-neutral-900 p-6">
              ...
              <div className="mt-6 flex justify-center">
                <QRCode
                  url={`https://neon-nightlife.com/hoppa/${venue.slug}`}
