...
export default function VenueSeoContent({ venue }: { venue: Venue }) {
  ...
  // ── 완독 보상 팁 (업소별 고유) ──
  const hiddenTips = [
    `예약 전화는 방문 1~2일 전이 여유 있다. 원하는 자리나 인원은 그때 미리 말해두면 준비가 수월하다.`,
    `첫 방문이면 예약 전화에서 인원과 방문 시간대를 알려두면 자리 안내가 빠르다.`,
    `금요일보다 목요일 밤이 진짜다. 금요일은 사람만 많고 정작 분위기는 목요일이 훨씬 낫다.`,
    `2~3명 단위가 자리 잡기도 편하고 대화하기에도 좋다. 큰 단체는 미리 예약해 두는 편이 안전하다.`,
