...
export default function VenueSeoContent({ venue }: { venue: Venue }) {
  ...
  // If content is too short, add a bonus reflection paragraph
  const totalLen = paragraphs.join('').length;
  if (totalLen < 1000) {
    const bonusTemplates = [
      `밤이라는 시간은 낮과 다른 규칙으로 움직인다. 낮에는 효율을 따지고 계산을 하지만, 밤에는 감각이 앞선다. 좋은 공간은 그 감각을 깨워주는 곳이다. ${vName}이(가) 기억에 남는 건 단순히 시설이 좋아서가 아니다. 그 안에서 보낸 시간의 질감이 남달랐기 때문이다. 누군가와 함께한 웃음, 혼자만의 생각, 잔을 기울이며 나눈 이야기. 그런 것들이 쌓여서 하나의 장소가 "내 곳"이 된다.`,
      `사람마다 밤을 보내는 방식이 다르다. 누군가는 시끄러운 곳에서 에너지를 얻고, 누군가는 조용한 곳에서 충전한다. 중요한 건 자기에게 맞는 곳을 찾는 일이다. 그 탐색의 과정이 귀찮다면, 검증된 곳부터 가보는 게 요령이다. 여기는 적어도 "가볼 만했다"는 말이 나오는 곳이다. 그 이상은 직접 확인하는 수밖에.`,
      `공간에 대한 판단은 결국 주관적이다. 같은 곳을 가도 어떤 사람은 만족하고 어떤 사람은 아쉬워한다. 하지만 오래 살아남는 곳에는 이유가 있다. 단골이 쌓이고, 입소문이 돌고, 처음 온 사람이 다시 오겠다고 말하는 곳. 그런 곳은 무언가를 제대로 하고 있다는 뜻이다. 글로 전하는 데는 한계가 있으니, 한번 가보기를 권한다.`,
    ];
    paragraphs.splice(paragraphs.length - 1, 0, bonusTemplates[seed % bonusTemplates.length]);
  }
