...
export default function StickyPhoneBar({ phone, staffName, venueName }: StickyPhoneBarProps) {
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 border-t border-neon-border bg-white/95 backdrop-blur-md shadow-lg">
        <div className="mx-auto flex h-14 md:h-16 max-w-7xl items-center justify-between px-4 sm:px-6">
          ...
          >
            ...
            <span className="hidden sm:inline">{staffName ? `${staffName}에게 전화 ${phone}` : phone}</span>
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 border-t border-neon-border bg-white/95 backdrop-blur-md shadow-lg">
      <div className="mx-auto flex h-14 md:h-16 max-w-7xl items-center justify-center px-4 sm:px-6">
        <p className="text-sm md:text-base font-bold text-neon-text">
          문의 &nbsp;
          <span className="rounded-lg bg-neon-primary/10 px-3 py-1 text-neon-primary">카톡 besta12</span>
        </p>
      </div>
    </div>
  );
