...
const ALL_MISSIONS: MissionDef[] = [
  ...
  { id: 'quiz',      icon: <Flame className="w-4 h-4" />,  title: 'MBTI 퀴즈 완료',       description: '나의 밤문화 MBTI를 알아보세요',             reward: 30, goal: 1, href: '/quiz' },
                ...
                <div className="space-y-3">
                  {todayMissions.map((mission, idx) => {
                    ...
                    if (!done && mission.href) {
                      return (
                        <Link key={mission.id} to={mission.href} onClick={() => setOpen(false)} className="block">
                          {card}
                        </Link>
                      );
                    }
