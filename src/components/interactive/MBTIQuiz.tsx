...
const QUESTIONS = [
  ...
  { q: '나이트라이프를 즐길 때 중요한 건?', a: ['새로운 사람 만나기', '함께 온 사람과 깊은 대화'], dim: 'EI' },
  { q: '음악 장르 선택은?', a: ['EDM·힙합 등 트렌디한 것', '트로트·발라드 등 익숙한 것'], dim: 'SN' },
  { q: '모임에서 당신의 역할은?', a: ['분위기 메이커', '조용히 즐기는 관찰자'], dim: 'TF' },
  { q: '이상적인 밤의 마무리는?', a: ['아침까지 달리기', '적당한 시간에 깔끔하게'], dim: 'JP' },
  { q: '여행지에서 나이트라이프를 즐긴다면?', a: ['현지 핫플 무조건 도전', '안전하고 검증된 곳만'], dim: 'JP' },
];

const TYPES: Record<string, { title: string; desc: string; venues: string[] }> = {
  ESTJ: { title: '파티 총괄 매니저', desc: '체계적으로 계획 세워서 빈틈 없는 밤을 만드는 타입. 친구들의 일정부터 장소까지 당신이 정합니다.', venues: ['강남청담클럽 레이스', '수원찬스돔나이트'] },
  ESTP: { title: '자유로운 클러버', desc: '즉흥적이고 에너지 넘치는 당신! EDM 비트에 몸을 맡기고 새벽까지 달리는 게 최고.', venues: ['강남청담클럽 사운드', '홍대클럽 버뮤다'] },
  ENFJ: { title: '분위기 메이커', desc: '함께하는 사람들을 행복하게 만드는 당신. 모두가 즐거운 밤을 보내도록 이끕니다.', venues: ['일산명월관요정', '강남줄리아나나이트'] },
  ENFP: { title: '모험하는 파티피플', desc: '새로운 곳을 탐험하고 온갖 사람을 만나는 걸 좋아하는 당신!', venues: ['이태원클럽 유토피아', '압구정클럽 하입'] },
  ISTJ: { title: '단골 VIP', desc: '검증된 장소를 깊게 파는 타입. 한번 마음에 들면 꾸준히 방문하는 진성 단골.', venues: ['일산룸', '해운대고구려'] },
  ISTP: { title: '쿨한 관찰자', desc: '조용히 분위기를 즐기며 음악과 술을 음미하는 타입.', venues: ['압구정라운지 디엠', '일산명월관요정'] },
  INFJ: { title: '감성 라운지족', desc: '깊은 대화와 좋은 음악이 있는 조용한 공간을 선호하는 당신.', venues: ['압구정이디엇라운지', '일산명월관요정'] },
  INFP: { title: '감성 야행성', desc: '분위기 좋은 곳에서 음악에 빠져드는 몽상가 타입.', venues: ['압구정라운지 디엠', '홍대클럽 퍼시픽'] },
};
...
export default function MBTIQuiz() {
  ...
  const badge = useMemo(() => {
    ...
    try {
      ...
      return quizCount === 1 ? '🎓 나이트라이프 연구생' : quizCount >= 5 ? '🎖️ MBTI 마스터' : quizCount >= 3 ? '📊 분석가' : null;
    } catch { return null; }
  }, [result]);
