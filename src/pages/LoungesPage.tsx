    ...
    <div className="mx-auto max-w-[1200px] px-4 py-8 sm:px-6 space-y-12">
      <div>
        ...
        <div className="rounded-2xl border border-neon-border/50 bg-neon-surface/30 p-6 space-y-4">
          ...
          <p className="text-base leading-relaxed text-neon-text-muted">
            라운지는 청담 일대에 집중되어 있다. 라운지 VIP 전용 부스는 미리 연락제로 운영하고, 부스마다 전담 스태프가 붙는다. 바에서 앉으면 바텐더가 취향을 물어보고 시그니처 드링크를 만들어준다. 위스키 셀렉션이 100종 넘는 곳도 있고, 내추럴 와인이나 크래프트 진토닉에 특화된 곳도 있다.
          </p>
          <p className="text-base leading-relaxed text-neon-text-muted">
            1인으로 와서 바에서 앉는 사람도 많다. 오히려 라운지는 1인이 오는 게 자연스러운 곳이다. 한 잔 비용 1.5~3만 원 정도. 세미포멀 편한 차림이면 어디든 들어간다. 금토 저녁은 만석이 잦으니 당일 전화로 빈자리 확인하는 게 안전하다.
          </p>
          <p className="text-base leading-relaxed text-neon-text-muted">
            압구정라운지 디엠은 초대장 없으면 못 들어가는 시크릿 바로 유명하다. 바텐더가 직접 만드는 시그니처 칵테일이 20종 이상이고, 좌석마다 전담 스태프가 붙는다. 압구정이디엇라운지는 이름부터 반전인 숨겨진 스피크이지 콘셉트다. 간판 없는 입구를 찾아 들어가면 1920년대 뉴욕 바를 재현한 인테리어가 나온다. 두 곳 다 비즈니스 접대나 소개팅 장소로 인기가 높고, 조용하면서도 격이 느껴지는 공간이다. 주중에는 워크인이 가능하지만 금토는 반드시 예약해야 한다.
          </p>
        </div>
      </div>
