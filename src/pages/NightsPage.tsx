...
export default function NightsPage() {
  useDocumentMeta('나이트 부킹 한 번도 못 잡고 집 간 적? 10년 웨이터가 거를 곳 알려줘요', '부킹 안 잡히는 나이트 가면 1차로 끝나요. 10년 짠밥 웨이터가 홀·부스·물·진행 다 풀어드립니다. 합석 시간·매너·드레스코드까지 121곳 갈 곳 vs 거를 곳 한 줄로 정리 →');
  ...
  return (
    <div className="mx-auto max-w-[1200px] px-4 py-8 sm:px-6 space-y-12">
      ...
      <div className="rounded-2xl border border-neon-border/50 bg-neon-surface/30 p-6 space-y-4">
          ...
          <p className="text-base leading-relaxed text-neon-text-muted">
            라이브 밴드가 트로트·팝·발라드를 번갈아 연주한다. 무대 바로 앞 댄스홀에서 춤추는 사람들, 자리에 앉아 양주 마시며 구경하는 사람들, 웨이터가 분주하게 오가는 모습이 한 공간에 섞인다. 30대 후반부터 50대까지 연령대가 넓다.
          </p>
