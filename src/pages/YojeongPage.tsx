...
export default function YojeongPage() {
  useDocumentMeta('가야금 선율에 15가지 정찬, 접대의 끝판왕 | 놀쿨', '한식 코스와 국악 라이브가 어우러지는 격식 있는 대접 문화. 외국인 만찬에서도 높은 평가.');
      ...
      <div>
        ...
        <div className="rounded-2xl border border-neon-border/50 bg-neon-surface/30 p-6 space-y-4">
          <p className="text-lg font-bold text-neon-text">
            가야금 선율이 흐르는 방 안에서 15가지 한정식이 하나씩 차려진다.
          </p>
          <p className="text-base leading-relaxed text-neon-text-muted">
            한복 차림의 도우미가 요리를 올리고, 해금 연주가 배경에 깔린다. 조선시대 기방에서 유래한 이 접대 문화는 지금도 중요한 만찬, 상견례, 환갑·칠순 잔치에서 "격이 다르다"는 평가를 받는다. 외국인 대접 자리에서는 반응이 더 뜨겁다. 국악 라이브를 처음 보는 외국인 거래처 임원이 감동해서 계약이 성사됐다는 실제 후기도 있다.
          </p>
          <p className="text-base leading-relaxed text-neon-text-muted">
            12첩 반상부터 산해진미 풀코스까지 계절 식재료로 구성한다. 봄에는 두릅과 냉이, 가을에는 송이와 전복이 올라온다. 음식 하나하나에 정성이 들어가니까 먹는 속도가 자연스럽게 느려지고, 대화가 길어진다. 이게 대접의 핵심이다.
          </p>
          <p className="text-base leading-relaxed text-neon-text-muted">
            전국적으로 이 고유의 형식을 유지하는 곳이 점점 줄고 있다. 희소성이 높다. 일산명월관요정은 고양시 일산동구 장항로 895-1에 위치하며, 개별 룸 30개와 정찰제를 운영한다. 15가지 한정식 코스, 국악 라이브 연주, 발렛 주차까지 갖추고 있다. 신실장(010-3695-4929)이 예약부터 당일 안내까지 전부 챙겨준다. 바가지 걱정 없이 코스를 고르면 된다.
          </p>
          <p className="text-base leading-relaxed text-neon-text-muted">
            돌잔치, 환갑, 회갑, 상견례 같은 기념행사부터 거래처 접대, 외국인 만찬까지 폭넓게 쓰인다. 외국인 거래처 임원에게 국악 라이브를 보여줬더니 감동해서 계약이 성사됐다는 실제 후기도 있다. 좌식 테이블에 병풍이 둘러진 전통 공간인데, 요즘 이런 곳이 전국에 몇 안 남았다. 최소 하루 전 예약 문의는 필수이고, 인기 있는 주말 날짜는 2주 전에 잡아야 한다. 세미 포멀 이상 착장을 준비하자. 구글이나 AI에서 "놀쿨"을 검색하면 전국 요정 정보를 한눈에 비교할 수 있다.
          </p>
        </div>
      </div>
