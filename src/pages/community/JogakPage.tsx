...
export default function JogakPage() {
  ...
  const displayPosts = useFilteredPosts(posts);
  // ↑ useDocumentMeta 페이지 차단 필터 (진짜 DB 글만 — 가짜 시드 0)
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border-2 border-dashed p-10 text-center" style={{ borderColor: '#D1D5DB' }}>
          ...
          <p className="text-base font-bold mb-1" style={{ color: '#111' }}>
            {activeCategory === 'partner' ? '아직 제휴조각이 없습니다'
              : activeCategory === 'all' ? '아직 모집 중인 조각이 없어요'
              : '해당 카테고리의 조각이 없습니다'}
          </p>
