...
const REGION_CAT_BODY: Record<string, { lead: string; paras: string[]; sub: string }> = {
  '대전|night': {
    ...
    paras: [
      '둔산역에서 5분 걸어 들어가면 트로트와 K팝 사이를 넘나드는 현장이 바로 둔산동 라인입니다. 출장 직장인이 퇴근 후 가장 먼저 찾는 동선이고, 트로트 라이브가 도는 사이마다 합석이 붙습니다. 차분하게 또래끼리 어울리고 싶으면 중앙로역 5분의 38세+ 전용 홀이 답인데, 50평 규모라 막내 실장이 입장부터 자리까지 직접 챙기고 평일 새벽 2시 반, 주말 3시 반까지 돌아갑니다.',
      '인원과 시간대를 먼저 정하면 고르기 쉽습니다. 출장·접대로 빠르게 합석을 트고 싶으면 둔산동 대형 홀, 동년배끼리 편하게 대화하며 노는 밤이면 중앙로 아담한 홀, 온천 관광 끝에 들르는 코스면 유성 쪽이 자연스럽습니다. 영업시간·입장 조건은 업소마다 다르니 방문 전 각 상세 페이지에서 한 번 더 확인하세요.',
    ],
    sub: '대전 어느 동네부터 고를까',
  },
  '청주|night': {
    ...
    paras: [
      '돈텔마마 쪽은 시내 중심가에 있어 찾기 쉽고 비교적 젊은 손님 비중이 높습니다. 같은 시내라도 호박 쪽은 7080 트로트 메들리가 길게 이어져 좀 더 전통적인 분위기를 원하는 단골이 모입니다. 성안길 일대는 웨이터가 합석을 빠르게 잡아주는 회전이 특징이라, 혼자 와도 자리에 앉고 30분이면 옆 테이블과 묶이는 흐름이 자연스럽습니다.',
