...
const REGION_CAT_PHONE: Record<string, { phone: string; staffName: string; venueName: string }> = {
  '대전|night': { phone: '', staffName: '', venueName: '대전원나이트' },
};
...
const REGION_CAT_BODY: Record<string, { lead: string; paras: string[]; sub: string }> = {
  '대전|night': {
    ...
    paras: [
      '둔산역에서 5분 걸어 들어가면 트로트와 K팝 사이를 넘나드는 현장이 바로 둔산동 라인입니다. 출장 직장인이 퇴근 후 가장 먼저 찾는 동선이고, 트로트 라이브가 도는 사이마다 플로어가 채워집니다. 차분하게 또래끼리 어울리고 싶으면 중앙로역 5분의 38세+ 전용 홀이 답인데, 50평 규모라 입장부터 자리까지 직접 챙기고 평일 새벽 2시 반, 주말 3시 반까지 돌아갑니다.',
