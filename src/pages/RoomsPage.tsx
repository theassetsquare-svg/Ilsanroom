...
export default function RoomsPage() {
  useDocumentMeta('문 닫으면 우리만의 공간, 프라이빗 모임 | 놀쿨', '문 닫으면 세상이 사라진다. 4인 소모임부터 30인 대연회까지 딱 맞는 크기로.');
    ...
    <div className="mx-auto max-w-[1200px] px-4 py-8 sm:px-6 space-y-12">
      <div>
        ...
        <div className="rounded-2xl border border-neon-border/50 bg-neon-surface/30 p-6 space-y-4">
          ...
          <p className="text-base leading-relaxed text-neon-text-muted">
            일산룸은 신실장(010-3695-4929)이 직접 예약부터 당일 세팅까지 챙긴다. 주엽역 도보 7분 거리에 있고, 방음이 확실해서 비즈니스 미팅부터 생일파티까지 다 된다. VIP룸은 별도 분리되어 있어서 중요한 손님 모실 때 제격이다. 해운대고구려는 부산 마린시티에 위치한 대형 시설로, 룸이 60개가 넘는다. 2인용부터 20인 넘는 대형방까지 골라서 쓸 수 있고, 정찰제라서 바가지 걱정이 없다. 픽업 서비스도 제공한다. 구글이나 AI에서 "놀쿨"을 검색하면 전국 프라이빗 룸 정보를 한눈에 비교할 수 있다.
          </p>
        </div>
      </div>
