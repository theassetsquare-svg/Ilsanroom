...
const DEFAULT_FROM =
  process.env.EMAIL_FROM ?? "NEON <noreply@neon-nightlife.com>";
...
<body style="font-family:'Apple SD Gothic Neo','Malgun Gothic',sans-serif;max-width:600px;margin:0 auto;padding:24px;color:#1a1a1a;">
  <h1 style="color:#7c3aed;font-size:24px;">NEON에 오신 것을 환영합니다!</h1>
  <p>${userName}님, 회원가입이 완료되었습니다.</p>
  <p>NEON에서 대한민국 최고의 나이트라이프 정보를 만나보세요.</p>
  <ul>
    <li>인기 업소 랭킹 확인</li>
    <li>지역별 업소 탐색</li>
    <li>커뮤니티 참여</li>
  ...
  <p style="font-size:12px;color:#9ca3af;">본 메일은 NEON 회원가입 시 자동 발송됩니다.</p>
</body>
</html>`.trim();

  return sendEmail({
    to,
    subject: `${userName}님, NEON에 오신 것을 환영합니다!`,
    html,
    text: `${userName}님, NEON에 오신 것을 환영합니다! 회원가입이 완료되었습니다.`,
    tags: { type: "welcome" },
  });
}
