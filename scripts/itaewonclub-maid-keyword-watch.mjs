/**
 * 이태원클럽 메이드 단일 키워드 SEO 24h watch — 시즌145.
 * 매일 KST 13:15 — title/desc/body 회귀시만 메일.
 *
 * 측정 키워드:
 *   "이태원클럽" — 단일 (지역+업종 결합형)
 *
 ...
 *  10) 디테일 토큰 (이태원역 1번 출구·지하 2층·미군 기지·올드스쿨·밀착 플로어) 모두 등장
 *  11) "이태원" + "클럽" 동시 흡수 (검색 분리어 보조)
 *     — Jack(시즌143 강남 90년대 아카이브)과 외국인/지하 페르소나로 분리
 */
...
const URL = 'https://nolcool.com/clubs/itaewon/itaewonclub-maid/';
const KEYWORD = '이태원클럽';
const DETAIL_TOKENS = ['이태원역 1번 출구', '지하 2층', '미군 기지', '올드스쿨', '밀착'];

function fetchHtml(url) {
  return new Promise((res) => {
    const t = setTimeout(() => res({ status: 0, html: '' }), 20000);
    https.get(url, { headers: { 'User-Agent': 'NolcoolMaidWatch/1.0' } }, r => {
      const chunks = [];
...
async function main() {
  ...
  console.log('이태원클럽 메이드 SEO watch');
  console.log('  URL:', URL);
...
async function sendMail({ issues, snapshot }) {
  ...
  const html = `<div style="font-family:sans-serif;max-width:680px;margin:0 auto;padding:20px">
    <h2 style="color:#DC2626">[⚠ 이태원클럽 메이드 SEO 회귀] ${issues.length}건</h2>
    ...
    <p style="color:#9CA3AF;font-size:11px;margin-top:20px">매일 KST 13:15 자동 — itaewonclub-maid-keyword-watch.mjs</p>
  </div>`;
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [TO],
      subject: `[놀쿨][⚠] 이태원클럽 메이드 SEO 회귀 ${issues.length}건`,
      html,
    }),
  });
