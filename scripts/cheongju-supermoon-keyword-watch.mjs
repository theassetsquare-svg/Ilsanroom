/**
 * 청주클럽 슈퍼문 단일 키워드 SEO 24h watch — 시즌149.
 * 매일 KST 13:35 — title/desc/body 회귀시만 메일.
 *
 * 측정 키워드:
 *   "청주클럽" — 단일
 *
...
function fetchHtml(url) {
  ...
  const _once = () => new Promise((res) => {
      ...
      https.get(url, { headers: { 'User-Agent': 'NolcoolSupermoonWatch/1.0' } }, r => {
        const chunks = [];
...
async function main() {
  ...
  const titleSplitHit = title.includes('청주') && title.includes('클럽');

  if (!titleHasKw) issues.push(`title "${KEYWORD}" 흡수 X`);
  if (!titleSplitHit) issues.push('title 청주+클럽 분리어 동시 흡수 X');
  ...
  console.log('청주클럽 슈퍼문 SEO watch');
  console.log('  URL:', URL);
...
async function sendMail({ issues, snapshot }) {
  ...
  const html = `<div style="font-family:sans-serif;max-width:680px;margin:0 auto;padding:20px">
    <h2 style="color:#DC2626">[⚠ 청주클럽 슈퍼문 SEO 회귀] ${issues.length}건</h2>
    ...
    <p style="color:#9CA3AF;font-size:11px;margin-top:20px">매일 KST 13:35 자동 — cheongju-supermoon-keyword-watch.mjs</p>
  </div>`;
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [TO],
      subject: `[놀쿨][⚠] 청주클럽 슈퍼문 SEO 회귀 ${issues.length}건`,
      html,
    }),
  });
