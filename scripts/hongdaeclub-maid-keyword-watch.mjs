/**
 * 홍대클럽 메이드 단일 키워드 SEO 24h watch — 시즌151.
 * 매일 KST 13:45 — title/desc/body 회귀시만 메일.
 *
 * 측정 키워드:
 *   "홍대클럽" — 단일
 *
 ...
 *  10) 디테일 토큰 (양화로23길·빈티지 카페 반지하·K-POP 리믹스·형광 팔찌·상수역 독막로 7분) 모두 등장
 *  11) "홍대" + "클럽" 동시 흡수 (검색 분리어 보조)
 *     — 시즌150(도깨비)과 5/5 디테일 토큰 0 겹침 차별화
 */
...
const URL = 'https://nolcool.com/clubs/hongdae/hongdaeclub-maid/';
const KEYWORD = '홍대클럽';
const DETAIL_TOKENS = ['양화로23길', '빈티지 카페', 'K-POP', '형광 팔찌', '상수역'];
...
async function main() {
  ...
  console.log('홍대클럽 메이드 SEO watch');
  console.log('  URL:', URL);
...
async function sendMail({ issues, snapshot }) {
  ...
  const html = `<div style="font-family:sans-serif;max-width:680px;margin:0 auto;padding:20px">
    <h2 style="color:#DC2626">[⚠ 홍대클럽 메이드 SEO 회귀] ${issues.length}건</h2>
    ...
    <p style="color:#9CA3AF;font-size:11px;margin-top:20px">매일 KST 13:45 자동 — hongdaeclub-maid-keyword-watch.mjs</p>
  </div>`;
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [TO],
      subject: `[놀쿨][⚠] 홍대클럽 메이드 SEO 회귀 ${issues.length}건`,
      html,
    }),
  });
