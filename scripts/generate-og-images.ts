...
function generateSvg(title: string, subtitle: string, bg: string, staff?: string): string {
  ...
  <text x="600" y="540" text-anchor="middle" font-family="sans-serif" font-size="20" fill="rgba(255,255,255,0.4)">오늘밤어디 — ilsanroom.pages.dev</text>
</svg>`;
}
...
// 1. Main page
fs.writeFileSync(path.join(outDir, 'main.svg'), generateSvg('오늘밤어디', '전국 클럽·나이트·라운지·룸·요정·호빠', '#8B5CF6'));

// 2. Category pages
for (const [cat, label] of Object.entries(categoryLabels)) {
  const color = categoryColors[cat] || '#8B5CF6';
  fs.writeFileSync(path.join(outDir, `${cat === 'club' ? 'clubs' : cat === 'night' ? 'nights' : cat === 'lounge' ? 'lounges' : cat === 'room' ? 'rooms' : cat}.svg`), generateSvg(`${label} 전체보기`, '오늘밤어디', color));
}
...
for (const v of venues) {
  ...
  fs.writeFileSync(path.join(outDir, `${v.slug}.svg`), generateSvg(v.nameKo, `${label} | 오늘밤어디`, color, staffText));
}
...
for (const p of utilPages) {
  fs.writeFileSync(path.join(outDir, `${p.file}.svg`), generateSvg(p.title, '오늘밤어디', p.bg));
}
