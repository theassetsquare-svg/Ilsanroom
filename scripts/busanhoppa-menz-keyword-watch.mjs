/**
 * 부산호빠 맨즈 페이지 2 키워드 SEO 24h watch (시즌117).
 * 매일 KST 10:40 — title/desc/h1/body 검색 의도 매칭 측정 → 회귀시만 메일.
 *
 * 측정 키워드:
 *   ① "부산호빠" — venue 지역+업종 (PRIMARY)
 *   ② "해운대호빠" — 인접 광역 (SECONDARY, 사용자 직접 명시, 해운대→광안리 차로 10분 코스 동선)
 *
 * 10지표 + 디테일 토큰:
 *   1) HTTP 200
 *   2) title에 "부산호빠" 포함
 ...
 *  +) 디테일 토큰 (광안리역/광안리해변/전신거울/암막/부산사투리) 모두 등장
 *
 * 환경:
 *   RESEND_API_KEY     필수
...
function fetchHtml(url) {
  return new Promise((res) => {
    ...
    https.get(url, { headers: { 'User-Agent': 'NolcoolBusanMenzWatch/1.0' } }, r => {
      const chunks = [];
...
async function sendMail({ issues, snapshot }) {
  ...
  const html = `<div style="font-family:sans-serif;max-width:680px;margin:0 auto;padding:20px">
    <h2 style="color:#DC2626">[⚠ 부산호빠 맨즈 SEO 회귀] ${issues.length}건</h2>
    ...
    <p style="color:#9CA3AF;font-size:11px;margin-top:20px">매일 KST 10:40 자동 — busanhoppa-menz-keyword-watch.mjs</p>
  </div>`;
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [TO],
      subject: `[놀쿨][⚠] 부산호빠 맨즈 SEO 회귀 ${issues.length}건`,
      html,
    }),
  });
