/**
 * 장안동호빠 빵빵 페이지 단일 키워드 SEO 24h watch (시즌122).
 * 매일 KST 11:20 — title/desc/body 검색 의도 매칭 측정 → 회귀시만 메일.
 *
 * 측정 키워드:
 *   "장안동호빠" (단일, 사용자 지정)
 *
 ...
 *   5) desc에 "장안동호빠" 포함
 *   6) desc ≤150자
 *   7) desc에 "장안동호빠" ≥1회 (shortDesc 지역접두사 제거 정책으로 2회→1회)
 *   8) "장안동호빠" body 등장 ≥5회
 *   9) "장안동호빠" body 밀도 1.0%~3.5%
 *  10) 후킹 5축 ≥1축 (title 또는 desc)
 *  11) 디테일 토큰 (장한평역·답십리역·곱창골목·한천로·동대문구청) 모두 등장
 *     — 시즌X 플렉스(4번출구·중랑천·미러볼·청량리·양꼬치)와 micro 분리
 *
 * 환경:
 *   RESEND_API_KEY     필수
...
const URL = 'https://nolcool.com/hoppa/jangandonghoppa-bbangbbang/';
const KEYWORD = '장안동호빠';
const DETAIL_TOKENS = ['장한평역', '답십리역', '곱창골목', '한천로', '동대문구청'];
...
function fetchHtml(url) {
  ...
  const _once = () => new Promise((res) => {
      ...
      https.get(url, { headers: { 'User-Agent': 'NolcoolJanganBbangWatch/1.0' } }, r => {
        const chunks = [];
...
async function main() {
  ...
  const bodyCount = (text.match(new RegExp(KEYWORD, 'g')) || []).length;
  const descCount = (desc.match(new RegExp(KEYWORD, 'g')) || []).length;
  const density = (bodyCount * KEYWORD.length) / text.length;
  ...
  if (!title.includes(KEYWORD)) issues.push(`title 장안동호빠X`);
  ...
  if (!desc.includes(KEYWORD)) issues.push(`desc 장안동호빠X`);
  if (desc.length === 0 || desc.length > 150) issues.push(`desc ${desc.length}자`);
  if (descCount < 1) issues.push(`desc 장안동호빠 ${descCount}회 (≥1 필요)`);
  if (bodyCount < 5) issues.push(`장안동호빠 body ${bodyCount}회 (≥5 필요)`);
  if (density < 0.01 || density > 0.035) issues.push(`장안동호빠 밀도 ${(density*100).toFixed(2)}% (1.0~3.5% 필요)`);
  ...
  const snapshot = {
    title, desc, bodyCount, descCount,
  ...
  console.log('장안동호빠 빵빵 SEO watch');
  console.log('  URL:', URL);
  console.log('  title:', title);
  console.log('  desc:', desc);
  console.log('  장안동호빠:', bodyCount, '회 / 밀도', snapshot.density, '/ desc', descCount, '회');
...
async function sendMail({ issues, snapshot }) {
  ...
  const html = `<div style="font-family:sans-serif;max-width:680px;margin:0 auto;padding:20px">
    <h2 style="color:#DC2626">[⚠ 장안동호빠 빵빵 SEO 회귀] ${issues.length}건</h2>
    ...
    <table style="border-collapse:collapse;width:100%;font-size:12px">
      ...
      <tr><td style="border:1px solid #E5E7EB;padding:6px">장안동호빠 body</td><td style="border:1px solid #E5E7EB;padding:6px">${snapshot.bodyCount}회 / 밀도 ${esc(snapshot.density || '')}</td></tr>
      <tr><td style="border:1px solid #E5E7EB;padding:6px">장안동호빠 desc</td><td style="border:1px solid #E5E7EB;padding:6px">${snapshot.descCount}회</td></tr>
    ...
    <p style="color:#9CA3AF;font-size:11px;margin-top:20px">매일 KST 11:20 자동 — jangandonghoppa-bbangbbang-keyword-watch.mjs</p>
  </div>`;
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [TO],
      subject: `[놀쿨][⚠] 장안동호빠 빵빵 SEO 회귀 ${issues.length}건`,
      html,
    }),
  });
