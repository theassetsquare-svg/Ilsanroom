/**
 * 해운대호빠 벨벳 페이지 2 키워드 SEO 24h watch (시즌126).
 * 매일 KST 11:40 — title/desc/body 검색 의도 매칭 측정 → 회귀시만 메일.
 *
 * 측정 키워드:
 *   ① "해운대호빠" — venue 지역+업종 (PRIMARY, 사용자 1순위 명시)
 *   ② "부산호빠"  — 광역 흡수 (SECONDARY, 사용자 명시)
 *
 * 12지표 + 디테일 토큰:
 *   1) HTTP 200
 *   2) title에 "해운대호빠" 포함
 ...
 *  11) 디테일 토큰 (장산역·해운대해수욕장·청사포·미포·부산아쿠아리움) 모두 등장
 *     — 해운대 5거점 중 벨벳 only lock, 해운대호빠 1순위 + 부산호빠 광역 동시 의도
 *  12) "해운대호빠" body 등장 ≥5회
 *
 * 환경:
 *   RESEND_API_KEY     필수
...
const URL = 'https://nolcool.com/hoppa/haeundaehoppa-velvet/';
const PRIMARY = '해운대호빠';
const SECONDARY = '부산호빠';
const DETAIL_TOKENS = ['장산역', '해운대해수욕장', '청사포', '미포', '부산아쿠아리움'];
...
function fetchHtml(url) {
  ...
  const _once = () => new Promise((res) => {
      ...
      https.get(url, { headers: { 'User-Agent': 'NolcoolVelvetWatch/1.0' } }, r => {
        const chunks = [];
...
async function main() {
  ...
  console.log('해운대호빠 벨벳 SEO watch');
  console.log('  URL:', URL);
...
async function sendMail({ issues, snapshot }) {
  ...
  const html = `<div style="font-family:sans-serif;max-width:680px;margin:0 auto;padding:20px">
    <h2 style="color:#DC2626">[⚠ 해운대호빠 벨벳 SEO 회귀] ${issues.length}건</h2>
    ...
    <p style="color:#9CA3AF;font-size:11px;margin-top:20px">매일 KST 11:40 자동 — haeundaehoppa-velvet-keyword-watch.mjs</p>
  </div>`;
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [TO],
      subject: `[놀쿨][⚠] 해운대호빠 벨벳 SEO 회귀 ${issues.length}건`,
      html,
    }),
  });
