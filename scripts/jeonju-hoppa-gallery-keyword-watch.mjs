/**
 * 전주호빠 갤러리 페이지 단일 키워드 SEO 24h watch (시즌124).
 * 매일 KST 11:30 — title/desc/body 검색 의도 매칭 측정 → 회귀시만 메일.
 *
 * 측정 키워드:
 *   "전주호빠" (단일, 사용자 지정)
 *
 ...
 *   5) desc에 "전주호빠" 포함
 *   6) desc ≤150자
 *   7) desc에 "전주호빠" ≥1회 (shortDesc 지역접두사 제거 정책으로 2회→1회)
 *   8) "전주호빠" body 등장 ≥5회
 *   9) "전주호빠" body 밀도 1.0%~3.5%
 *  10) 후킹 5축 ≥1축 (title 또는 desc)
 *  11) 디테일 토큰 (전주역·한옥마을·객리단길·전동성당·풍남문) 모두 등장
 *     — 전주 5대 명소 lock, 전주호빠 1순위 검색 의도 직격
 *
 * 환경:
 *   RESEND_API_KEY     필수
...
const URL = 'https://nolcool.com/hoppa/jeonju-hoppa-gallery/';
const KEYWORD = '전주호빠';
const DETAIL_TOKENS = ['전주역', '한옥마을', '객리단길', '전동성당', '풍남문'];
...
function fetchHtml(url) {
  ...
  const _once = () => new Promise((res) => {
      ...
      https.get(url, { headers: { 'User-Agent': 'NolcoolJeonjuGalleryWatch/1.0' } }, r => {
        const chunks = [];
...
async function main() {
  ...
  if (!title.includes(KEYWORD)) issues.push(`title 전주호빠X`);
  ...
  if (!desc.includes(KEYWORD)) issues.push(`desc 전주호빠X`);
  if (desc.length === 0 || desc.length > 150) issues.push(`desc ${desc.length}자`);
  if (descCount < 1) issues.push(`desc 전주호빠 ${descCount}회 (≥1 필요)`);
  if (bodyCount < 5) issues.push(`전주호빠 body ${bodyCount}회 (≥5 필요)`);
  if (density < 0.01 || density > 0.035) issues.push(`전주호빠 밀도 ${(density*100).toFixed(2)}% (1.0~3.5% 필요)`);
  ...
  console.log('전주호빠 갤러리 SEO watch');
  console.log('  URL:', URL);
  console.log('  title:', title);
  console.log('  desc:', desc);
  console.log('  전주호빠:', bodyCount, '회 / 밀도', snapshot.density, '/ desc', descCount, '회');
...
async function sendMail({ issues, snapshot }) {
  ...
  const html = `<div style="font-family:sans-serif;max-width:680px;margin:0 auto;padding:20px">
    <h2 style="color:#DC2626">[⚠ 전주호빠 갤러리 SEO 회귀] ${issues.length}건</h2>
    ...
    <table style="border-collapse:collapse;width:100%;font-size:12px">
      ...
      <tr><td style="border:1px solid #E5E7EB;padding:6px">전주호빠 body</td><td style="border:1px solid #E5E7EB;padding:6px">${snapshot.bodyCount}회 / 밀도 ${esc(snapshot.density || '')}</td></tr>
      <tr><td style="border:1px solid #E5E7EB;padding:6px">전주호빠 desc</td><td style="border:1px solid #E5E7EB;padding:6px">${snapshot.descCount}회</td></tr>
    ...
    <p style="color:#9CA3AF;font-size:11px;margin-top:20px">매일 KST 11:30 자동 — jeonju-hoppa-gallery-keyword-watch.mjs</p>
  </div>`;
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [TO],
      subject: `[놀쿨][⚠] 전주호빠 갤러리 SEO 회귀 ${issues.length}건`,
      html,
    }),
  });
