/**
 * 제주나이트 페이지 2 키워드 SEO 24h watch (시즌104).
 * 매일 KST 09:35 — title/desc/h1/body 검색 의도 매칭 측정 → 회귀시만 메일.
 *
 * 측정 키워드 (전국 유일 제주 venue, 사용자 검색 의도 dual):
 *   ① "제주도나이트" — venue 정식명 (제주도 = 섬, 사용자 핵심 검색)
 *   ② "제주나이트"    — 도-생략 변형 (흡수 인정: 제주도나이트 ⊃ 제주+나이트)
 *
 * 10지표 + 디테일 토큰:
 *   1) HTTP 200
 *   2) title에 "제주도나이트" 포함
 *   3) title이 "제주나이트" 흡수 (제주+나이트 토큰)
 *   4) title ≤60자
 *   5) title 중복단어 없음
 *   6) desc 150자 이내 + "제주도나이트" 포함
 *   7) desc가 "제주나이트" 흡수
 *   8) "제주도나이트" 밀도 ≤3.5%
 *   9) body가 "제주나이트" 흡수 (제주+나이트)
 *  10) 후킹 5축 ≥1축 (title 또는 desc)
 *  +) 디테일 토큰 (흑돼지/서귀포/관광객/트로트/합석) 모두 등장
 *
 * 환경:
 *   RESEND_API_KEY     필수
...
const URL = 'https://nolcool.com/nights/jejunight/';
const PRIMARY = '제주도나이트';
const SECONDARY = '제주나이트';
const DETAIL_TOKENS = ['흑돼지', '서귀포', '관광객', '트로트', '합석'];
...
function fetchHtml(url) {
  ...
  const _once = () => new Promise((res) => {
      ...
      https.get(url, { headers: { 'User-Agent': 'NolcoolJejuNightWatch/1.0' } }, r => {
        const chunks = [];
...
async function main() {
  ...
  if (!title.includes(PRIMARY)) issues.push(`title 제주도나이트X`);
  if (!absorbsSecondary(title, SECONDARY)) issues.push(`title 제주나이트 흡수X`);
  ...
  if (!desc.includes(PRIMARY)) issues.push(`desc 제주도나이트X`);
  if (desc.length === 0 || desc.length > 150) issues.push(`desc ${desc.length}자`);
  if (!absorbsSecondary(desc, SECONDARY)) issues.push(`desc 제주나이트 흡수X`);
  if (primaryDensity > 0.035) issues.push(`제주도나이트 밀도 ${(primaryDensity*100).toFixed(2)}%`);
  if (!absorbsSecondary(text, SECONDARY)) issues.push(`body 제주나이트 흡수X`);
  ...
  console.log('제주도나이트 SEO watch');
  console.log('  URL:', URL);
  console.log('  title:', title);
  console.log('  desc:', desc);
  console.log('  제주도나이트:', primaryCount, '회 / 밀도', snapshot.primaryDensity);
  console.log('  제주나이트(흡수):', absorbsSecondary(text, SECONDARY) ? 'O' : 'X', '· 리터럴', secondaryCount, '회');
...
async function sendMail({ issues, snapshot }) {
  ...
  const html = `<div style="font-family:sans-serif;max-width:680px;margin:0 auto;padding:20px">
    <h2 style="color:#DC2626">[⚠ 제주도나이트 SEO 회귀] ${issues.length}건</h2>
    ...
    <table style="border-collapse:collapse;width:100%;font-size:12px">
      ...
      <tr><td style="border:1px solid #E5E7EB;padding:6px">제주도나이트</td><td style="border:1px solid #E5E7EB;padding:6px">${snapshot.primaryCount}회 / 밀도 ${esc(snapshot.primaryDensity || '')}</td></tr>
      <tr><td style="border:1px solid #E5E7EB;padding:6px">제주나이트(흡수)</td><td style="border:1px solid #E5E7EB;padding:6px">리터럴 ${snapshot.secondaryCount}회</td></tr>
    ...
    <p style="color:#9CA3AF;font-size:11px;margin-top:20px">매일 KST 09:35 자동 — jejunight-keyword-watch.mjs</p>
  </div>`;
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [TO],
      subject: `[놀쿨][⚠] 제주도나이트 SEO 회귀 ${issues.length}건`,
      html,
    }),
  });
