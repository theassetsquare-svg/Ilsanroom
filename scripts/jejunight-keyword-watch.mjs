 ...
 *   ① "제주나이트"    — venue 정식명 (브랜드 검색)
 *   ② "제주도나이트" — 사용자 일반 검색 키워드 (수동 주입)
 *
 * 10지표 + 디테일 토큰:
 *   1) HTTP 200
 *   2) title에 "제주나이트" 포함
...
async function main() {
  ...
  if (!title.includes(PRIMARY)) issues.push(`title 제주나이트X`);
  if (!absorbsSecondary(title, SECONDARY)) issues.push(`title 제주도나이트X`);
  ...
  if (!desc.includes(PRIMARY)) issues.push(`desc 제주나이트X`);
  if (desc.length === 0 || desc.length > 150) issues.push(`desc ${desc.length}자`);
  if (!desc.includes(SECONDARY)) issues.push(`desc 제주도나이트X`);
  if (primaryDensity > 0.035) issues.push(`제주나이트 밀도 ${(primaryDensity*100).toFixed(2)}%`);
  if (secondaryCount < 3) issues.push(`제주도나이트 body ${secondaryCount}회 (≥3 필요)`);
  ...
  console.log('제주나이트 SEO watch');
  console.log('  URL:', URL);
  console.log('  title:', title);
  console.log('  desc:', desc);
  console.log('  제주나이트:', primaryCount, '회 / 밀도', snapshot.primaryDensity);
  console.log('  제주도나이트:', secondaryCount, '회');
...
async function sendMail({ issues, snapshot }) {
  ...
  const html = `<div style="font-family:sans-serif;max-width:680px;margin:0 auto;padding:20px">
    <h2 style="color:#DC2626">[⚠ 제주나이트 SEO 회귀] ${issues.length}건</h2>
    ...
    <table style="border-collapse:collapse;width:100%;font-size:12px">
      ...
      <tr><td style="border:1px solid #E5E7EB;padding:6px">제주나이트</td><td style="border:1px solid #E5E7EB;padding:6px">${snapshot.primaryCount}회 / 밀도 ${esc(snapshot.primaryDensity || '')}</td></tr>
      <tr><td style="border:1px solid #E5E7EB;padding:6px">제주도나이트</td><td style="border:1px solid #E5E7EB;padding:6px">${snapshot.secondaryCount}회</td></tr>
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [TO],
      subject: `[놀쿨][⚠] 제주나이트 SEO 회귀 ${issues.length}건`,
      html,
    }),
  });
