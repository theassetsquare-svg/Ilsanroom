/**
 * 홍대클럽 버뮤다 단일 키워드 SEO 24h watch — 시즌152.
 * 매일 KST 13:50 — title/desc/body 회귀시만 메일.
 *
 * 11지표:
 *   1) HTTP 200
 *   2) title에 "홍대클럽" 흡수
 ...
 *  10) 디테일 토큰 (세 개의 방·프로그레시브 하우스·연남동 경의선숲길·난파선 오브제·SOS 칵테일) 모두 등장
 *  11) "홍대" + "클럽" 분리어 동시 흡수
 *     — 시즌150(도깨비)/151(메이드)와 5/5 디테일 토큰 0 겹침
 */
...
const URL = 'https://nolcool.com/clubs/hongdae/hongdaeclub-bermuda/';
const KEYWORD = '홍대클럽';
const DETAIL_TOKENS = ['세 개의 방', '프로그레시브 하우스', '연남동 경의선숲길', '난파선 오브제', 'SOS 칵테일'];
...
function fetchHtml(url) {
  ...
  const _once = () => new Promise((res) => {
      ...
      https.get(url, { headers: { 'User-Agent': 'NolcoolBermudaWatch/1.0' } }, r => {
        const chunks = [];
...
async function main() {
  ...
  console.log('홍대클럽 버뮤다 SEO watch');
  console.log('  URL:', URL);
...
async function sendMail({ issues, snapshot }) {
  ...
  const html = `<div style="font-family:sans-serif;max-width:680px;margin:0 auto;padding:20px">
    <h2 style="color:#DC2626">[⚠ 홍대클럽 버뮤다 SEO 회귀] ${issues.length}건</h2>
    ...
    <p style="color:#9CA3AF;font-size:11px;margin-top:20px">매일 KST 13:50 자동 — hongdaeclub-bermuda-keyword-watch.mjs</p>
  </div>`;
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [TO],
      subject: `[놀쿨][⚠] 홍대클럽 버뮤다 SEO 회귀 ${issues.length}건`,
      html,
    }),
  });
