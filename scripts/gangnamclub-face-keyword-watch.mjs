/**
 * 강남청담클럽 페이스 DUAL 키워드 SEO 24h watch — 시즌141.
 * 매일 KST 12:55 — title/desc/body 회귀시만 메일.
 *
 * 측정 키워드:
 ...
 *  12) 디테일 토큰 (청담사거리·보고타 DJ·바차타 슬로우·라임 4쪽·야자수 벽화) 모두 등장
 *     — 라퓨타·레이스·미로·밤앤밤·사운드·아르떼·아르쥬·유토피아 0/40 겹침: 라틴/살사 페르소나 + 보고타 DJ 격주 시그니처
 */
...
const URL = 'https://nolcool.com/clubs/gangnam/gangnamclub-face/';
const PRIMARY = '강남클럽';
const SECONDARY = '청담클럽';
const DETAIL_TOKENS = ['청담사거리', '보고타', '바차타 슬로우', '라임 4쪽', '야자수 벽화'];

function fetchHtml(url) {
  return new Promise((res) => {
    const t = setTimeout(() => res({ status: 0, html: '' }), 20000);
    https.get(url, { headers: { 'User-Agent': 'NolcoolFaceWatch/1.0' } }, r => {
      const chunks = [];
...
async function main() {
  ...
  console.log('강남청담클럽 페이스 SEO watch');
  console.log('  URL:', URL);
...
async function sendMail({ issues, snapshot }) {
  ...
  const html = `<div style="font-family:sans-serif;max-width:680px;margin:0 auto;padding:20px">
    <h2 style="color:#DC2626">[⚠ 강남청담클럽 페이스 SEO 회귀] ${issues.length}건</h2>
    ...
    <p style="color:#9CA3AF;font-size:11px;margin-top:20px">매일 KST 12:55 자동 — gangnamclub-face-keyword-watch.mjs</p>
  </div>`;
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [TO],
      subject: `[놀쿨][⚠] 강남청담클럽 페이스 SEO 회귀 ${issues.length}건`,
      html,
    }),
  });
