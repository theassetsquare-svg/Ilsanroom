/**
 * 강남청담클럽 밤앤밤 DUAL 키워드 SEO 24h watch — 시즌136.
 * 매일 KST 12:30 — title/desc/body 회귀시만 메일.
 *
 * 측정 키워드:
 *   ① "강남클럽" — PRIMARY (강남 광역 검색)
 *   ② "청담클럽" — SECONDARY (청담 마이크로 검색)
 *   venue name "강남청담클럽 밤앤밤"가 양쪽 키워드 흡수 + desc/body 명시 보강
 *
 * 12지표:
 *   1) HTTP 200
 ...
 *  12) 디테일 토큰 (신사역·시티팝·크래프트 맥주 탭·미러볼 3개·목요 프리뷰 나이트) 모두 등장
 *     — 라퓨타·레이스·미로와 0/5 겹침: 강남클럽 캐주얼 클러버 검색 의도 차별화 (시티팝→퓨처 베이스 트랜지션 시그니처)
 *
 * 환경:
 *   RESEND_API_KEY     필수
...
const URL = 'https://nolcool.com/clubs/gangnam/gangnamclub-bamnbam/';
const PRIMARY = '강남클럽';
...
async function main() {
  ...
  // venue name "강남청담클럽 밤앤밤"이 강남+클럽 동시 흡수 — 부분 매칭 검사
  ...
  console.log('강남청담클럽 밤앤밤 SEO watch');
  console.log('  URL:', URL);
...
async function sendMail({ issues, snapshot }) {
  ...
  const html = `<div style="font-family:sans-serif;max-width:680px;margin:0 auto;padding:20px">
    <h2 style="color:#DC2626">[⚠ 강남청담클럽 밤앤밤 SEO 회귀] ${issues.length}건</h2>
    ...
    <p style="color:#9CA3AF;font-size:11px;margin-top:20px">매일 KST 12:30 자동 — gangnamclub-bamnbam-keyword-watch.mjs</p>
  </div>`;
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [TO],
      subject: `[놀쿨][⚠] 강남청담클럽 밤앤밤 SEO 회귀 ${issues.length}건`,
      html,
    }),
  });
