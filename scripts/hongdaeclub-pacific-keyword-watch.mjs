/**
 * 홍대클럽 퍼시픽 단일 키워드 SEO 24h watch — 시즌153.
 * 매일 KST 13:55 — title/desc/body 회귀시만 메일.
 *
 * 11지표:
 *   1) HTTP 200
 *   2) title에 "홍대클럽" 흡수
 ...
 *  10) 디테일 토큰 (1번 출구·Funktion-One·천장 5미터·드럼앤베이스·오션 블루 하이볼) 모두 등장
 *  11) "홍대" + "클럽" 분리어 동시 흡수
 *     — 시즌150(도깨비)/151(메이드)/152(버뮤다)와 5/5 디테일 토큰 0 겹침
 */
...
const URL = 'https://nolcool.com/clubs/hongdae/hongdaeclub-pacific/';
const KEYWORD = '홍대클럽';
const DETAIL_TOKENS = ['1번 출구', 'Funktion-One', '천장 5미터', '드럼앤베이스', '오션 블루 하이볼'];
...
function fetchHtml(url) {
  ...
  const _once = () => new Promise((res) => {
      ...
      https.get(url, { headers: { 'User-Agent': 'NolcoolPacificWatch/1.0' } }, r => {
        const chunks = [];
...
async function main() {
  ...
  console.log('홍대클럽 퍼시픽 SEO watch');
  console.log('  URL:', URL);
...
async function sendMail({ issues, snapshot }) {
  ...
  const html = `<div style="font-family:sans-serif;max-width:680px;margin:0 auto;padding:20px">
    <h2 style="color:#DC2626">[⚠ 홍대클럽 퍼시픽 SEO 회귀] ${issues.length}건</h2>
    ...
    <p style="color:#9CA3AF;font-size:11px;margin-top:20px">매일 KST 13:55 자동 — hongdaeclub-pacific-keyword-watch.mjs</p>
  </div>`;
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [TO],
      subject: `[놀쿨][⚠] 홍대클럽 퍼시픽 SEO 회귀 ${issues.length}건`,
      html,
    }),
  });
