/**
 * 해운대호빠 깐따삐야 페이지 2 키워드 SEO 24h watch (시즌110).
 * 매일 KST 10:05 — title/desc/h1/body 검색 의도 매칭 측정 → 회귀시만 메일.
 *
 * 측정 키워드:
 *   ① "해운대호빠" — venue 지역+업종 (PRIMARY, SSR auto-secondary와 일치)
 *   ② "부산호빠"   — 광역 검색 키워드 (SECONDARY, shortDesc/description 수동 주입)
 *
 * 10지표 + 디테일 토큰:
 *   1) HTTP 200
 *   2) title에 "해운대호빠" 포함
...
function fetchHtml(url) {
  return new Promise((res) => {
    ...
    https.get(url, { headers: { 'User-Agent': 'NolcoolHaeundaeKkantappiyaWatch/1.0' } }, r => {
      const chunks = [];
...
async function main() {
  ...
  console.log('해운대호빠 깐따삐야 SEO watch');
  console.log('  URL:', URL);
...
async function sendMail({ issues, snapshot }) {
  ...
  const html = `<div style="font-family:sans-serif;max-width:680px;margin:0 auto;padding:20px">
    <h2 style="color:#DC2626">[⚠ 해운대호빠 깐따삐야 SEO 회귀] ${issues.length}건</h2>
    ...
    <p style="color:#9CA3AF;font-size:11px;margin-top:20px">매일 KST 10:05 자동 — haeundaehoppa-kkantappiya-keyword-watch.mjs</p>
  </div>`;
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [TO],
      subject: `[놀쿨][⚠] 해운대호빠 깐따삐야 SEO 회귀 ${issues.length}건`,
      html,
    }),
  });
