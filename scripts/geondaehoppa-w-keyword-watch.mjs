/**
 * 건대호빠 W 페이지 2 키워드 SEO 24h watch (시즌114).
 * 매일 KST 10:25 — title/desc/h1/body 검색 의도 매칭 측정 → 회귀시만 메일.
 *
 * 측정 키워드:
 *   ① "건대호빠" — venue 지역+업종 (PRIMARY)
 *   ② "서울호빠" — 광역 검색 (SECONDARY, shortDesc/description 수동 주입)
 *
 * 10지표 + 디테일 토큰:
 *   1) HTTP 200
 *   2) title에 "건대호빠" 포함
 ...
 *   4) title ≤60자
 *   5) title 중복단어 없음
 *   6) desc 150자 이내 + "건대호빠" 포함
 *   7) desc에 "서울호빠" ≥3회
 *   8) "건대호빠" 밀도 ≤3.5%
 *   9) "서울호빠" body 등장 ≥3회
 *  10) 후킹 5축 ≥1축 (title 또는 desc)
 *  +) 디테일 토큰 (건대입구역/커먼그라운드/화양동/24세/미니멀) 모두 등장
 *
 * 환경:
 *   RESEND_API_KEY     필수
...
const URL = 'https://nolcool.com/hoppa/geondaehoppa-w/';
const PRIMARY = '건대호빠';
const SECONDARY = '서울호빠';
...
async function main() {
  ...
  if (!title.includes(PRIMARY)) issues.push(`title 건대호빠X`);
  if (!title.includes(SECONDARY)) issues.push(`title 서울호빠X`);
  ...
  if (!desc.includes(PRIMARY)) issues.push(`desc 건대호빠X`);
  ...
  if (primaryDensity > 0.035) issues.push(`건대호빠 밀도 ${(primaryDensity*100).toFixed(2)}%`);
  ...
  console.log('건대호빠 W SEO watch');
  console.log('  URL:', URL);
  console.log('  title:', title);
  console.log('  desc:', desc);
  console.log('  건대호빠:', primaryCount, '회 / 밀도', snapshot.primaryDensity);
  console.log('  서울호빠:', secondaryCount, '회 (desc', secondaryInDesc, '회)');
...
async function sendMail({ issues, snapshot }) {
  ...
  const html = `<div style="font-family:sans-serif;max-width:680px;margin:0 auto;padding:20px">
    <h2 style="color:#DC2626">[⚠ 건대호빠 W SEO 회귀] ${issues.length}건</h2>
    ...
    <table style="border-collapse:collapse;width:100%;font-size:12px">
      ...
      <tr><td style="border:1px solid #E5E7EB;padding:6px">건대호빠</td><td style="border:1px solid #E5E7EB;padding:6px">${snapshot.primaryCount}회 / 밀도 ${esc(snapshot.primaryDensity || '')}</td></tr>
      <tr><td style="border:1px solid #E5E7EB;padding:6px">서울호빠</td><td style="border:1px solid #E5E7EB;padding:6px">${snapshot.secondaryCount}회 (desc ${snapshot.secondaryInDesc}회)</td></tr>
    ...
    <p style="color:#9CA3AF;font-size:11px;margin-top:20px">매일 KST 10:25 자동 — geondaehoppa-w-keyword-watch.mjs</p>
  </div>`;
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [TO],
      subject: `[놀쿨][⚠] 건대호빠 W SEO 회귀 ${issues.length}건`,
      html,
    }),
  });
