/**
 * 강남호빠 로얄 페이지 2 키워드 SEO 24h watch (시즌111).
 * 매일 KST 10:10 — title/desc/h1/body 검색 의도 매칭 측정 → 회귀시만 메일.
 *
 * 측정 키워드:
 ...
 *  +) 디테일 토큰 (강남역/왕관/샹들리에/벨벳/크라운) 모두 등장
 *
 * 환경:
 *   RESEND_API_KEY     필수
...
const URL = 'https://nolcool.com/hoppa/gangnamhoppa-royal/';
const PRIMARY = '강남호빠';
const SECONDARY = '서울호빠';
const DETAIL_TOKENS = ['강남역', '왕관', '샹들리에', '벨벳', '크라운'];

function fetchHtml(url) {
  return new Promise((res) => {
    const t = setTimeout(() => res({ status: 0, html: '' }), 20000);
    https.get(url, { headers: { 'User-Agent': 'NolcoolGangnamRoyalWatch/1.0' } }, r => {
      const chunks = [];
...
async function sendMail({ issues, snapshot }) {
  ...
  const html = `<div style="font-family:sans-serif;max-width:680px;margin:0 auto;padding:20px">
    <h2 style="color:#DC2626">[⚠ 강남호빠 로얄 SEO 회귀] ${issues.length}건</h2>
    ...
    <p style="color:#9CA3AF;font-size:11px;margin-top:20px">매일 KST 10:10 자동 — gangnamhoppa-royal-keyword-watch.mjs</p>
  </div>`;
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [TO],
      subject: `[놀쿨][⚠] 강남호빠 로얄 SEO 회귀 ${issues.length}건`,
      html,
    }),
  });
