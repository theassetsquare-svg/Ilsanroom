/**
 * 강남청담클럽 레이스 DUAL 키워드 SEO 24h watch — 시즌133.
 * 매일 KST 12:20 — title/desc/body 회귀시만 메일.
 *
 * 측정 키워드:
 *   ① "강남클럽" — PRIMARY (강남 광역 검색)
 *   ② "청담클럽" — SECONDARY (청담 마이크로 검색)
 *   venue name "강남청담클럽 레이스"가 양쪽 키워드 흡수 + desc/body 명시 보강
 *
 * 12지표:
 *   1) HTTP 200
 ...
 *  12) 디테일 토큰 (코엑스·BPM 140·펑타임 라인 어레이·발코니석·봉은사로) 모두 등장
 *     — 라푸타와 0/5 겹침: 강남클럽 마니아 검색 의도 차별화 (위치·BPM·시그니처 사운드)
 *
 * 환경:
 *   RESEND_API_KEY     필수
...
const URL = 'https://nolcool.com/clubs/gangnam/gangnamclub-race/';
const PRIMARY = '강남클럽';
const SECONDARY = '청담클럽';
const DETAIL_TOKENS = ['코엑스', 'BPM 140', '펑타임 라인 어레이', '발코니석', '봉은사로'];

function fetchHtml(url) {
  return new Promise((res) => {
    const t = setTimeout(() => res({ status: 0, html: '' }), 20000);
    https.get(url, { headers: { 'User-Agent': 'NolcoolRaceWatch/1.0' } }, r => {
      const chunks = [];
...
async function main() {
  ...
  // venue name "강남청담클럽 레이스"가 강남+클럽 동시 흡수 — 부분 매칭 검사
  ...
  console.log('강남청담클럽 레이스 SEO watch');
  console.log('  URL:', URL);
...
async function sendMail({ issues, snapshot }) {
  ...
  const html = `<div style="font-family:sans-serif;max-width:680px;margin:0 auto;padding:20px">
    <h2 style="color:#DC2626">[⚠ 강남청담클럽 레이스 SEO 회귀] ${issues.length}건</h2>
    ...
    <p style="color:#9CA3AF;font-size:11px;margin-top:20px">매일 KST 12:20 자동 — gangnamclub-race-keyword-watch.mjs</p>
  </div>`;
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [TO],
      subject: `[놀쿨][⚠] 강남청담클럽 레이스 SEO 회귀 ${issues.length}건`,
      html,
    }),
  });
