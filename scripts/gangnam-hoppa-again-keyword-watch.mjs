/**
 * 강남호빠 어게인 페이지 2 키워드 SEO 24h watch (시즌112).
 * 매일 KST 10:15 — title/desc/h1/body 검색 의도 매칭 측정 → 회귀시만 메일.
 *
 * 측정 키워드:
 ...
 *  +) 디테일 토큰 (삼성동/정실장/봉은사/화요일/프라이빗) 모두 등장
 *
 * 환경:
 *   RESEND_API_KEY     필수
...
const URL = 'https://nolcool.com/hoppa/gangnam-hoppa-again/';
const PRIMARY = '강남호빠';
const SECONDARY = '서울호빠';
const DETAIL_TOKENS = ['삼성동', '정실장', '봉은사', '화요일', '프라이빗'];

function fetchHtml(url) {
  return new Promise((res) => {
    const t = setTimeout(() => res({ status: 0, html: '' }), 20000);
    https.get(url, { headers: { 'User-Agent': 'NolcoolGangnamAgainWatch/1.0' } }, r => {
      const chunks = [];
...
async function main() {
  ...
  console.log('강남호빠 어게인 SEO watch');
  console.log('  URL:', URL);
...
async function sendMail({ issues, snapshot }) {
  ...
  const html = `<div style="font-family:sans-serif;max-width:680px;margin:0 auto;padding:20px">
    <h2 style="color:#DC2626">[⚠ 강남호빠 어게인 SEO 회귀] ${issues.length}건</h2>
    ...
    <p style="color:#9CA3AF;font-size:11px;margin-top:20px">매일 KST 10:15 자동 — gangnam-hoppa-again-keyword-watch.mjs</p>
  </div>`;
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [TO],
      subject: `[놀쿨][⚠] 강남호빠 어게인 SEO 회귀 ${issues.length}건`,
      html,
    }),
  });
