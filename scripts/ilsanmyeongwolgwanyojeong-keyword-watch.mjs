/**
 * 일산명월관요정 페이지 2 키워드 SEO 24h watch (시즌129).
 * 매일 KST 11:55 — title/desc/body 검색 의도 매칭 측정 → 회귀시만 메일.
 *
 * 측정 키워드:
 *   ① "일산명월관" — venue 브랜드 (PRIMARY, 사용자 1순위 명시)
 *   ② "일산요정"  — 지역+업종 광역 흡수 (SECONDARY, 사용자 명시)
 *
 * 12지표 + 디테일 토큰:
 *   1) HTTP 200
 *   2) title에 "일산명월관" 포함 (PRIMARY only; venue name "일산명월관요정"이 "일산요정" 검색 매칭 흡수)
 *   3) title 의미중복 없음 (같은 2자 stem 2회 등장 = 키워드 stuffing)
 *   4) title ≤60자
 *   5) title 중복단어 없음
 *   6) desc ≤150자 + "일산명월관" ≥3회
 *   7) desc에 "일산요정" ≥3회
 *   8) "일산명월관" body 밀도 ≤3.5%
 *   9) "일산요정" body 등장 ≥3회
...
async function main() {
  ...
  // 의미중복 검출: 같은 2자 stem이 2번 이상 (예: 일산명월관요정+일산요정)
  const stemMap = new Map();
  for (const w of titleWords) { const s = w.slice(0, 2); stemMap.set(s, (stemMap.get(s) || 0) + 1); }
...
async function sendMail({ issues, snapshot }) {
  ...
  const html = `<div style="font-family:sans-serif;max-width:680px;margin:0 auto;padding:20px">
    <h2 style="color:#DC2626">[⚠ 일산명월관요정 SEO 회귀] ${issues.length}건</h2>
    ...
    <p style="color:#9CA3AF;font-size:11px;margin-top:20px">매일 KST 11:55 자동 — ilsanmyeongwolgwanyojeong-keyword-watch.mjs</p>
  </div>`;
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [TO],
      subject: `[놀쿨][⚠] 일산명월관요정 SEO 회귀 ${issues.length}건`,
      html,
    }),
  });
