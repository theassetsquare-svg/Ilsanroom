/**
 * /yojeong 카테고리 페이지 단일 키워드 SEO 24h watch (시즌128).
 * 매일 KST 11:50 — title/desc/body 검색 의도 매칭 측정 → 회귀시만 메일.
 *
 * 측정 키워드:
 *   "요정" (단일, 사용자 지정)
 *
 ...
 *   6) desc ≤150자
 *   7) desc에 "요정" ≥2회
 *   8) "요정" body 등장 ≥20회 (카테고리 페이지 floor)
 *   9) "요정" body 밀도 1.0%~3.0%
 *  10) 후킹 5축 ≥1축 (title 또는 desc)
 *  11) 디테일 토큰 (한정식·국악·가야금·일산명월관·정찰제) 모두 등장
 *     — 요정 검색 의도(전통 정찬·접대·격식) 직격
 *
 * 환경:
 *   RESEND_API_KEY     필수
...
const URL = 'https://nolcool.com/yojeong/';
const KEYWORD = '요정';
const DETAIL_TOKENS = ['한정식', '국악', '가야금', '일산명월관', '정찰제'];

function fetchHtml(url) {
  return new Promise((res) => {
    const t = setTimeout(() => res({ status: 0, html: '' }), 20000);
    https.get(url, { headers: { 'User-Agent': 'NolcoolYojeongWatch/1.0' } }, r => {
      const chunks = [];
...
async function main() {
  ...
  if (!title.includes(KEYWORD)) issues.push(`title 요정X`);
  ...
  if (!desc.includes(KEYWORD)) issues.push(`desc 요정X`);
  if (desc.length === 0 || desc.length > 150) issues.push(`desc ${desc.length}자`);
  if (descCount < 2) issues.push(`desc 요정 ${descCount}회 (≥2 필요)`);
  if (bodyCount < 20) issues.push(`요정 body ${bodyCount}회 (≥20 필요)`);
  if (density < 0.01 || density > 0.03) issues.push(`요정 밀도 ${(density*100).toFixed(2)}% (1.0~3.0% 필요)`);
  ...
  console.log('/yojeong 카테고리 SEO watch');
  console.log('  URL:', URL);
  console.log('  title:', title);
  console.log('  desc:', desc);
  console.log('  요정:', bodyCount, '회 / 밀도', snapshot.density, '/ desc', descCount, '회');
...
async function sendMail({ issues, snapshot }) {
  ...
  const html = `<div style="font-family:sans-serif;max-width:680px;margin:0 auto;padding:20px">
    <h2 style="color:#DC2626">[⚠ /yojeong SEO 회귀] ${issues.length}건</h2>
    ...
    <table style="border-collapse:collapse;width:100%;font-size:12px">
      ...
      <tr><td style="border:1px solid #E5E7EB;padding:6px">요정 body</td><td style="border:1px solid #E5E7EB;padding:6px">${snapshot.bodyCount}회 / 밀도 ${esc(snapshot.density || '')}</td></tr>
      <tr><td style="border:1px solid #E5E7EB;padding:6px">요정 desc</td><td style="border:1px solid #E5E7EB;padding:6px">${snapshot.descCount}회</td></tr>
    ...
    <p style="color:#9CA3AF;font-size:11px;margin-top:20px">매일 KST 11:50 자동 — yojeong-keyword-watch.mjs</p>
  </div>`;
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [TO],
      subject: `[놀쿨][⚠] /yojeong SEO 회귀 ${issues.length}건`,
      html,
    }),
  });
