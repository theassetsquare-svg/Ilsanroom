/**
 * 부산호빠 아우라 페이지 2 키워드 SEO 24h watch (시즌119).
 * 매일 KST 11:05 — title/desc/body 검색 의도 매칭 측정 → 회귀시만 메일.
 *
 * 측정 키워드:
...
function fetchHtml(url) {
  ...
  const _once = () => new Promise((res) => {
      ...
      https.get(url, { headers: { 'User-Agent': 'NolcoolBusanAuraWatch/1.0' } }, r => {
        const chunks = [];
...
async function main() {
  ...
  console.log('부산호빠 아우라 SEO watch');
  console.log('  URL:', URL);
...
async function sendMail({ issues, snapshot }) {
  ...
  const html = `<div style="font-family:sans-serif;max-width:680px;margin:0 auto;padding:20px">
    <h2 style="color:#DC2626">[⚠ 부산호빠 아우라 SEO 회귀] ${issues.length}건</h2>
    ...
    <p style="color:#9CA3AF;font-size:11px;margin-top:20px">매일 KST 11:05 자동 — busanhoppa-aura-keyword-watch.mjs</p>
  </div>`;
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [TO],
      subject: `[놀쿨][⚠] 부산호빠 아우라 SEO 회귀 ${issues.length}건`,
      html,
    }),
  });
