/**
 * 독산동국빈관나이트 페이지 2 키워드 SEO 24h watch (시즌84).
 * 매일 KST 08:00 — title/desc/h1/body 검색 의도 매칭 측정 → 회귀시만 메일.
 *
 * 측정 키워드:
 *   ① "독산동국빈관나이트" — 정확 키워드 (브랜드 검색)
 *   ② "독산나이트"         — 일반 키워드 (지역 + 카테고리 검색)
 *
 * 10지표 + 디테일 토큰:
 *   1) HTTP 200
 *   2) title에 "독산동국빈관나이트" 포함
 *   3) title에 "독산나이트" 포함
 *   4) title ≤60자
 *   5) title 중복단어 없음
 *   6) desc 150자 이내 + "독산동국빈관나이트" 포함
 *   7) desc에 "독산나이트" 포함
 *   8) "독산동국빈관나이트" 밀도 ≤3.5%
 *   9) "독산나이트" body 등장 ≥3회
 *  10) 후킹 5축 ≥1축 (title 또는 desc)
 *  +) 디테일 토큰 (가산디지털단지/1호선/독산역/트로트/야근족) 모두 등장
 *
 * 환경:
 *   RESEND_API_KEY     필수
...
const URL = 'https://nolcool.com/nights/doksangukbingwannight/';
const PRIMARY = '독산동국빈관나이트';
const SECONDARY = '독산나이트';
const DETAIL_TOKENS = ['가산디지털단지', '1호선', '독산역', '트로트', '야근족'];
...
function fetchHtml(url) {
  ...
  const _once = () => new Promise((res) => {
      ...
      https.get(url, { headers: { 'User-Agent': 'NolcoolDoksanGukbingwanWatch/1.0' } }, r => {
        const chunks = [];
...
async function main() {
  ...
  if (!title.includes(PRIMARY)) issues.push(`title 독산동국빈관나이트X`);
  if (!absorbsSecondary(title, SECONDARY)) issues.push(`title 독산나이트X`);
  ...
  if (!desc.includes(PRIMARY)) issues.push(`desc 독산동국빈관나이트X`);
  if (desc.length === 0 || desc.length > 150) issues.push(`desc ${desc.length}자`);
  if (!absorbsSecondary(desc, SECONDARY)) issues.push(`desc 독산나이트X`);
  if (primaryDensity > 0.035) issues.push(`독산동국빈관나이트 밀도 ${(primaryDensity*100).toFixed(2)}%`);
  if (secondaryCount < 3) issues.push(`독산나이트 body ${secondaryCount}회 (≥3 필요)`);
  ...
  console.log('독산동국빈관나이트 SEO watch');
  console.log('  URL:', URL);
  console.log('  title:', title);
  console.log('  desc:', desc);
  console.log('  독산동국빈관나이트:', primaryCount, '회 / 밀도', snapshot.primaryDensity);
  console.log('  독산나이트:', secondaryCount, '회');
...
async function sendMail({ issues, snapshot }) {
  ...
  const html = `<div style="font-family:sans-serif;max-width:680px;margin:0 auto;padding:20px">
    <h2 style="color:#DC2626">[⚠ 독산동국빈관나이트 SEO 회귀] ${issues.length}건</h2>
    ...
    <table style="border-collapse:collapse;width:100%;font-size:12px">
      ...
      <tr><td style="border:1px solid #E5E7EB;padding:6px">독산동국빈관나이트</td><td style="border:1px solid #E5E7EB;padding:6px">${snapshot.primaryCount}회 / 밀도 ${esc(snapshot.primaryDensity || '')}</td></tr>
      <tr><td style="border:1px solid #E5E7EB;padding:6px">독산나이트</td><td style="border:1px solid #E5E7EB;padding:6px">${snapshot.secondaryCount}회</td></tr>
    ...
    <p style="color:#9CA3AF;font-size:11px;margin-top:20px">매일 KST 08:00 자동 — doksangukbingwannight-keyword-watch.mjs</p>
  </div>`;
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [TO],
      subject: `[놀쿨][⚠] 독산동국빈관나이트 SEO 회귀 ${issues.length}건`,
      html,
    }),
  });
