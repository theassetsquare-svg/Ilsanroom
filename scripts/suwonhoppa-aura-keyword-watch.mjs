/**
 * 수원호빠 아우라 페이지 단일 키워드 SEO 24h watch (시즌121).
 * 매일 KST 11:15 — title/desc/body 검색 의도 매칭 측정 → 회귀시만 메일.
 *
 * 측정 키워드:
 *   "수원호빠" (단일, 사용자 지정)
 *
 ...
 *  11) 디테일 토큰 (수원시청역·팔달문·수원천·통닭거리·아주대) 모두 등장
 *     — 시즌120 비스트(인계동·매교역·수원역·화성행궁·광교호수공원)와 0/5 토큰 겹침
 *
 * 환경:
 *   RESEND_API_KEY     필수
...
const URL = 'https://nolcool.com/hoppa/suwonhoppa-aura/';
const KEYWORD = '수원호빠';
const DETAIL_TOKENS = ['수원시청역', '팔달문', '수원천', '통닭거리', '아주대'];
...
function fetchHtml(url) {
  ...
  const _once = () => new Promise((res) => {
      ...
      https.get(url, { headers: { 'User-Agent': 'NolcoolSuwonAuraWatch/1.0' } }, r => {
        const chunks = [];
...
async function main() {
  ...
  if (!title.includes(KEYWORD)) issues.push(`title 수원호빠X`);
  ...
  console.log('수원호빠 아우라 SEO watch');
  console.log('  URL:', URL);
  console.log('  title:', title);
  console.log('  desc:', desc);
  console.log('  수원호빠:', bodyCount, '회 / 밀도', snapshot.density, '/ desc', descCount, '회');
...
async function sendMail({ issues, snapshot }) {
  ...
  const html = `<div style="font-family:sans-serif;max-width:680px;margin:0 auto;padding:20px">
    <h2 style="color:#DC2626">[⚠ 수원호빠 아우라 SEO 회귀] ${issues.length}건</h2>
    ...
    <table style="border-collapse:collapse;width:100%;font-size:12px">
      ...
      <tr><td style="border:1px solid #E5E7EB;padding:6px">수원호빠 body</td><td style="border:1px solid #E5E7EB;padding:6px">${snapshot.bodyCount}회 / 밀도 ${esc(snapshot.density || '')}</td></tr>
      <tr><td style="border:1px solid #E5E7EB;padding:6px">수원호빠 desc</td><td style="border:1px solid #E5E7EB;padding:6px">${snapshot.descCount}회</td></tr>
    ...
    <p style="color:#9CA3AF;font-size:11px;margin-top:20px">매일 KST 11:15 자동 — suwonhoppa-aura-keyword-watch.mjs</p>
  </div>`;
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [TO],
      subject: `[놀쿨][⚠] 수원호빠 아우라 SEO 회귀 ${issues.length}건`,
      html,
    }),
  });
