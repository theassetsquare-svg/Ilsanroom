/**
 * 강남청담클럽 아르떼 DUAL 키워드 SEO 24h watch — 시즌138.
 * 매일 KST 12:40 — title/desc/body 회귀시만 메일.
 *
 * 측정 키워드:
 *   ① "강남클럽" — PRIMARY (강남 광역 검색)
 *   ② "청담클럽" — SECONDARY (청담 마이크로 검색)
 *   venue name "강남청담클럽 아르떼"가 양쪽 키워드 흡수 + desc/body 명시 보강
 *
 * 12지표:
 *   1) HTTP 200
 ...
 *  12) 디테일 토큰 (압구정로데오역·갤러리 골목·누재즈·미디어 아티스트 설치 작품·내추럴 와인 12종) 모두 등장
 *     — 라퓨타·레이스·미로·밤앤밤·사운드와 0/25 겹침: 아트/누재즈 시그니처 차별화 (갤러리 큐레이션 메타포)
 *
 * 환경:
 *   RESEND_API_KEY     필수
...
const URL = 'https://nolcool.com/clubs/gangnam/gangnamclub-arte/';
const PRIMARY = '강남클럽';
const SECONDARY = '청담클럽';
const DETAIL_TOKENS = ['압구정로데오역', '갤러리 골목', '누재즈', '미디어 아티스트 설치 작품', '내추럴 와인 12종'];

function fetchHtml(url) {
  return new Promise((res) => {
    const t = setTimeout(() => res({ status: 0, html: '' }), 20000);
    https.get(url, { headers: { 'User-Agent': 'NolcoolArteWatch/1.0' } }, r => {
      const chunks = [];
...
async function main() {
  ...
  console.log('강남청담클럽 아르떼 SEO watch');
  console.log('  URL:', URL);
...
async function sendMail({ issues, snapshot }) {
  ...
  const html = `<div style="font-family:sans-serif;max-width:680px;margin:0 auto;padding:20px">
    <h2 style="color:#DC2626">[⚠ 강남청담클럽 아르떼 SEO 회귀] ${issues.length}건</h2>
    ...
    <p style="color:#9CA3AF;font-size:11px;margin-top:20px">매일 KST 12:40 자동 — gangnamclub-arte-keyword-watch.mjs</p>
  </div>`;
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [TO],
      subject: `[놀쿨][⚠] 강남청담클럽 아르떼 SEO 회귀 ${issues.length}건`,
      html,
    }),
  });
