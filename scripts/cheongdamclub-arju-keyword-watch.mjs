/**
 * 청담클럽 아르쥬 DUAL 키워드 SEO 24h watch — 시즌139.
 * 매일 KST 12:45 — title/desc/body 회귀시만 메일.
 *
 * 측정 키워드:
 *   ① "강남클럽" — PRIMARY (강남 광역 검색)
 *   ② "청담클럽" — SECONDARY (청담 마이크로 검색)
 *   venue name "강남청담클럽 아르쥬"가 양쪽 키워드 흡수 (청담 region 페이지)
 *
 * 12지표:
 *   1) HTTP 200
 *   2) title에 "강남" + "클럽" 동시 포함
 *   3) title ≤60자
 *   4) title 중복단어 없음
 *   5) desc 80~150자
 *   6) desc에 "강남클럽" ≥3회
 ...
 *  12) 디테일 토큰 (청담명품거리·프렌치 꼬냑 13종·멜로딕 하우스·샴페인 타워 스파클러·발레파킹 데스크) 모두 등장
 *     — 라퓨타·레이스·미로·밤앤밤·사운드·아르떼와 0/30 겹침: 청담 프렌치 럭셔리 시그니처
 *
 * 환경:
 *   RESEND_API_KEY     필수
...
const URL = 'https://nolcool.com/clubs/cheongdam/cheongdamclub-arju/';
const PRIMARY = '강남클럽';
const SECONDARY = '청담클럽';
const DETAIL_TOKENS = ['청담명품거리', '프렌치 꼬냑 13종', '멜로딕 하우스', '샴페인 타워 스파클러', '발레파킹 데스크'];

function fetchHtml(url) {
  return new Promise((res) => {
    const t = setTimeout(() => res({ status: 0, html: '' }), 20000);
    https.get(url, { headers: { 'User-Agent': 'NolcoolArjuWatch/1.0' } }, r => {
      const chunks = [];
...
async function main() {
  ...
  console.log('청담클럽 아르쥬 SEO watch');
  console.log('  URL:', URL);
...
async function sendMail({ issues, snapshot }) {
  ...
  const html = `<div style="font-family:sans-serif;max-width:680px;margin:0 auto;padding:20px">
    <h2 style="color:#DC2626">[⚠ 청담클럽 아르쥬 SEO 회귀] ${issues.length}건</h2>
    ...
    <p style="color:#9CA3AF;font-size:11px;margin-top:20px">매일 KST 12:45 자동 — cheongdamclub-arju-keyword-watch.mjs</p>
  </div>`;
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [TO],
      subject: `[놀쿨][⚠] 청담클럽 아르쥬 SEO 회귀 ${issues.length}건`,
      html,
    }),
  });
