/**
 * 수원호빠 비스트 페이지 단일 키워드 SEO 24h watch (시즌120).
 * 매일 KST 11:10 — title/desc/body 검색 의도 매칭 측정 → 회귀시만 메일.
 *
 * 측정 키워드:
 *   "수원호빠" (단일, 사용자 지정)
 *
...
function fetchHtml(url) {
  return new Promise((res) => {
    ...
    https.get(url, { headers: { 'User-Agent': 'NolcoolSuwonBeastWatch/1.0' } }, r => {
      const chunks = [];
...
async function main() {
  ...
  console.log('수원호빠 비스트 SEO watch');
  console.log('  URL:', URL);
...
async function sendMail({ issues, snapshot }) {
  ...
  const html = `<div style="font-family:sans-serif;max-width:680px;margin:0 auto;padding:20px">
    <h2 style="color:#DC2626">[⚠ 수원호빠 비스트 SEO 회귀] ${issues.length}건</h2>
    ...
    <p style="color:#9CA3AF;font-size:11px;margin-top:20px">매일 KST 11:10 자동 — suwonhoppa-beast-keyword-watch.mjs</p>
  </div>`;
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [TO],
      subject: `[놀쿨][⚠] 수원호빠 비스트 SEO 회귀 ${issues.length}건`,
      html,
    }),
  });
