/**
 * 강남청담클럽 사운드 DUAL 키워드 SEO 24h watch — 시즌137.
 * 매일 KST 12:35 — title/desc/body 회귀시만 메일.
 *
 * 측정 키워드:
 *   ① "강남클럽" — PRIMARY (강남 광역 검색)
 *   ② "청담클럽" — SECONDARY (청담 마이크로 검색)
 *   venue name "강남청담클럽 사운드"가 양쪽 키워드 흡수 + desc/body 명시 보강
 *
 * 12지표:
 *   1) HTTP 200
 ...
 *  12) 디테일 토큰 (학동역·Funktion-One·서브우퍼 6대·사운드 엔지니어 상주·토요 새벽 2시 피크) 모두 등장
 *     — 라퓨타·레이스·미로·밤앤밤과 0/5 겹침: 음향 시그니처 차별화 (Funktion-One 6스택 진동)
 *
 * 환경:
 *   RESEND_API_KEY     필수
...
function fetchHtml(url) {
  ...
  const _once = () => new Promise((res) => {
      ...
      https.get(url, { headers: { 'User-Agent': 'NolcoolSoundWatch/1.0' } }, r => {
        const chunks = [];
...
async function main() {
  ...
  console.log('강남청담클럽 사운드 SEO watch');
  console.log('  URL:', URL);
...
async function sendMail({ issues, snapshot }) {
  ...
  const html = `<div style="font-family:sans-serif;max-width:680px;margin:0 auto;padding:20px">
    <h2 style="color:#DC2626">[⚠ 강남청담클럽 사운드 SEO 회귀] ${issues.length}건</h2>
    ...
    <p style="color:#9CA3AF;font-size:11px;margin-top:20px">매일 KST 12:35 자동 — gangnamclub-sound-keyword-watch.mjs</p>
  </div>`;
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [TO],
      subject: `[놀쿨][⚠] 강남청담클럽 사운드 SEO 회귀 ${issues.length}건`,
      html,
    }),
  });
