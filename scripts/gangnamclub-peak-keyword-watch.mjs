/**
 * 강남청담클럽 피크 DUAL 키워드 SEO 24h watch — 시즌142.
 * 매일 KST 13:00 — title/desc/body 회귀시만 메일.
 *
 * 측정 키워드:
 ...
 *  12) 디테일 토큰 (선릉로·콘크리트 4층·천장 개방·빅룸 일렉트로·허브 인퓨즈드 보드카) 모두 등장
 *     — 라퓨타·레이스·미로·밤앤밤·사운드·아르떼·아르쥬·유토피아·페이스 0/45 겹침: 오픈에어 야경 페르소나 + 콘크리트 4층 시그니처
 */
...
const URL = 'https://nolcool.com/clubs/gangnam/gangnamclub-peak/';
const PRIMARY = '강남클럽';
const SECONDARY = '청담클럽';
const DETAIL_TOKENS = ['선릉로', '콘크리트', '천장 개방', '빅룸 일렉트로', '허브 인퓨즈드'];
...
function fetchHtml(url) {
  ...
  const _once = () => new Promise((res) => {
      ...
      https.get(url, { headers: { 'User-Agent': 'NolcoolPeakWatch/1.0' } }, r => {
        const chunks = [];
...
async function main() {
  ...
  console.log('강남청담클럽 피크 SEO watch');
  console.log('  URL:', URL);
...
async function sendMail({ issues, snapshot }) {
  ...
  const html = `<div style="font-family:sans-serif;max-width:680px;margin:0 auto;padding:20px">
    <h2 style="color:#DC2626">[⚠ 강남청담클럽 피크 SEO 회귀] ${issues.length}건</h2>
    ...
    <p style="color:#9CA3AF;font-size:11px;margin-top:20px">매일 KST 13:00 자동 — gangnamclub-peak-keyword-watch.mjs</p>
  </div>`;
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [TO],
      subject: `[놀쿨][⚠] 강남청담클럽 피크 SEO 회귀 ${issues.length}건`,
      html,
    }),
  });
