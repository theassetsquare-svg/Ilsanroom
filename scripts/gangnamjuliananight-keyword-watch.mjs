/**
 * 강남줄리아나나이트 페이지 2 키워드 SEO 24h watch (시즌81).
 * 매일 KST 07:45 — title/desc/h1/body 검색 의도 매칭 측정 → 회귀시만 메일.
 *
 * 측정 키워드:
 *   ① "강남줄리아나나이트" — 정확 키워드 (브랜드 검색)
 *   ② "강남나이트"         — 일반 키워드 (지역 + 카테고리 검색, 전국 최고 검색량)
 *
 * 10지표:
 *   1) HTTP 200
 *   2) title에 "강남줄리아나나이트" 포함
 *   3) title에 "강남나이트" 포함
 *   4) title ≤60자
 *   5) title 중복단어 없음
 *   6) desc 150자 이내 + "강남줄리아나나이트" 포함
 *   7) desc에 "강남나이트" 포함
 *   8) "강남줄리아나나이트" 밀도 ≤3.5% (스터핑 방지)
 *   9) "강남나이트" body 등장 ≥3회 (검색 의도 매칭)
...
const URL = 'https://nolcool.com/nights/gangnamjuliananight/';
const PRIMARY = '강남줄리아나나이트';
const SECONDARY = '강남나이트';
...
function fetchHtml(url) {
  ...
  const _once = () => new Promise((res) => {
      ...
      https.get(url, { headers: { 'User-Agent': 'NolcoolGangnamJulianaWatch/1.0' } }, r => {
        const chunks = [];
...
async function main() {
  ...
  if (!title.includes(PRIMARY)) issues.push(`title 강남줄리아나나이트X`);
  if (!absorbsSecondary(title, SECONDARY)) issues.push(`title 강남나이트X`);
  ...
  console.log('강남줄리아나나이트 SEO watch');
  console.log('  URL:', URL);
  console.log('  title:', title);
  console.log('  desc:', desc);
  console.log('  강남줄리아나나이트:', primaryCount, '회 / 밀도', snapshot.primaryDensity);
  console.log('  강남나이트:', secondaryCount, '회');
...
async function sendMail({ issues, snapshot }) {
  ...
  const html = `<div style="font-family:sans-serif;max-width:680px;margin:0 auto;padding:20px">
    <h2 style="color:#DC2626">[⚠ 강남줄리아나나이트 SEO 회귀] ${issues.length}건</h2>
    ...
    <table style="border-collapse:collapse;width:100%;font-size:12px">
      ...
      <tr><td style="border:1px solid #E5E7EB;padding:6px">강남줄리아나나이트</td><td style="border:1px solid #E5E7EB;padding:6px">${snapshot.primaryCount}회 / 밀도 ${esc(snapshot.primaryDensity || '')}</td></tr>
      <tr><td style="border:1px solid #E5E7EB;padding:6px">강남나이트</td><td style="border:1px solid #E5E7EB;padding:6px">${snapshot.secondaryCount}회</td></tr>
    ...
    <p style="color:#9CA3AF;font-size:11px;margin-top:20px">매일 KST 07:45 자동 — gangnamjuliananight-keyword-watch.mjs</p>
  </div>`;
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [TO],
      subject: `[놀쿨][⚠] 강남줄리아나나이트 SEO 회귀 ${issues.length}건`,
      html,
    }),
  });
