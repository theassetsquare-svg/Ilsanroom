/**
 * 강남청담클럽 Jack DUAL 키워드 SEO 24h watch — 시즌143.
 * 매일 KST 13:05 — title/desc/body 회귀시만 메일.
 *
 * 측정 키워드:
 ...
 *  12) 디테일 토큰 (영동대교·붉은 벽돌·바이닐 스크래치·피칸 스모크·수요 오픈 마이크) 모두 등장
 *     — 10 sibling 0/50 겹침: 힙합 아카이비스트 페르소나 + 바이닐 시그니처
 */
...
const URL = 'https://nolcool.com/clubs/gangnam/gangnamclub-jack/';
const PRIMARY = '강남클럽';
const SECONDARY = '청담클럽';
const DETAIL_TOKENS = ['영동대교', '붉은 벽돌', '바이닐 스크래치', '피칸 스모크', '수요 오픈 마이크'];

function fetchHtml(url) {
  return new Promise((res) => {
    const t = setTimeout(() => res({ status: 0, html: '' }), 20000);
    https.get(url, { headers: { 'User-Agent': 'NolcoolJackWatch/1.0' } }, r => {
      const chunks = [];
...
async function main() {
  ...
  console.log('강남청담클럽 Jack SEO watch');
  console.log('  URL:', URL);
...
async function sendMail({ issues, snapshot }) {
  ...
  const html = `<div style="font-family:sans-serif;max-width:680px;margin:0 auto;padding:20px">
    <h2 style="color:#DC2626">[⚠ 강남청담클럽 Jack SEO 회귀] ${issues.length}건</h2>
    ...
    <p style="color:#9CA3AF;font-size:11px;margin-top:20px">매일 KST 13:05 자동 — gangnamclub-jack-keyword-watch.mjs</p>
  </div>`;
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [TO],
      subject: `[놀쿨][⚠] 강남청담클럽 Jack SEO 회귀 ${issues.length}건`,
      html,
    }),
  });
