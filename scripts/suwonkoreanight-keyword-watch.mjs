/**
 * 수원코리아나이트 페이지 2 키워드 SEO 24h watch (시즌93).
 * 매일 KST 08:45 — title/desc/h1/body 검색 의도 매칭 측정 → 회귀시만 메일.
 *
 * 측정 키워드:
 *   ① "수원코리아나이트" — 정확 키워드 (브랜드 검색)
 *   ② "수원나이트"        — 일반 키워드 (지역 + 카테고리 검색)
 *
 * 10지표 + 디테일 토큰:
 *   1) HTTP 200
 *   2) title에 "수원코리아나이트" 포함
 *   3) title에 "수원나이트" 포함
 *   4) title ≤60자
 *   5) title 중복단어 없음
 *   6) desc 150자 이내 + "수원코리아나이트" 포함
 *   7) desc에 "수원나이트" 포함
 *   8) "수원코리아나이트" 밀도 ≤3.5%
 *   9) "수원나이트" body 등장 ≥3회
 *  10) 후킹 5축 ≥1축 (title 또는 desc)
 *  +) 디테일 토큰 (인계동/로데오/멤버십/발라드/합석) 모두 등장
 *
 * 환경:
 *   RESEND_API_KEY     필수
...
const URL = 'https://nolcool.com/nights/suwonkoreanight/';
const PRIMARY = '수원코리아나이트';
const SECONDARY = '수원나이트';
const DETAIL_TOKENS = ['인계동', '로데오', '멤버십', '발라드'];
...
function fetchHtml(url) {
  ...
  const _once = () => new Promise((res) => {
      ...
      https.get(url, { headers: { 'User-Agent': 'NolcoolSuwonKoreaWatch/1.0' } }, r => {
        const chunks = [];
...
async function main() {
  ...
  if (!title.includes(PRIMARY)) issues.push(`title 수원코리아나이트X`);
  ...
  if (!desc.includes(PRIMARY)) issues.push(`desc 수원코리아나이트X`);
  ...
  if (primaryDensity > 0.035) issues.push(`수원코리아나이트 밀도 ${(primaryDensity*100).toFixed(2)}%`);
  ...
  console.log('수원코리아나이트 SEO watch');
  console.log('  URL:', URL);
  console.log('  title:', title);
  console.log('  desc:', desc);
  console.log('  수원코리아나이트:', primaryCount, '회 / 밀도', snapshot.primaryDensity);
...
async function sendMail({ issues, snapshot }) {
  ...
  const html = `<div style="font-family:sans-serif;max-width:680px;margin:0 auto;padding:20px">
    <h2 style="color:#DC2626">[⚠ 수원코리아나이트 SEO 회귀] ${issues.length}건</h2>
    ...
    <table style="border-collapse:collapse;width:100%;font-size:12px">
      ...
      <tr><td style="border:1px solid #E5E7EB;padding:6px">수원코리아나이트</td><td style="border:1px solid #E5E7EB;padding:6px">${snapshot.primaryCount}회 / 밀도 ${esc(snapshot.primaryDensity || '')}</td></tr>
    ...
    <p style="color:#9CA3AF;font-size:11px;margin-top:20px">매일 KST 08:45 자동 — suwonkoreanight-keyword-watch.mjs</p>
  </div>`;
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [TO],
      subject: `[놀쿨][⚠] 수원코리아나이트 SEO 회귀 ${issues.length}건`,
      html,
    }),
  });
