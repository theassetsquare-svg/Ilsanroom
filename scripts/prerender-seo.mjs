...
for (const [tag, tagVenues] of Object.entries(allTags)) {
  ...
  const desc = `'${tag}' ${aggPick(tag, ['키워드로 묶인', '결로 모은', '태그가 붙은'], 11)} ${tagCats.join('·')} ${tagVenues.length}곳. ${tagTopNames} 등 ${tagRegions.slice(0, 3).join(', ')}${tagRegions.length > 3 ? ' 등' : ''} 업소를 ${aggPick(tag, ['같은 결끼리 모아 한눈에 비교합니다', '한 페이지에서 바로 비교하세요', '분위기·후기·전화번호까지 정리했습니다'], 12)}.`;
  let ssrBody = `<h1>${escHtml(title)}</h1><p>${escHtml(desc)}</p>`;
