...
const staticPages = [
  ...
  { path: '/clubs', title: '새벽 2시에도 줄이 안 줄어드는 클럽만 골랐다', desc: '강남 홍대 이태원 일산 EDM·힙합·테크노 클럽 35곳. 입장료 분위기 드레스코드 영업시간 한눈에. 첫방문 코스, 게스트 라인업, 부킹 문화까지. 오늘 밤 갈 곳 여기서 픽.' },
  ...
  { path: '/hoppa', title: '처음인데 혼자 가도 괜찮을까? 결론부터, 된다', desc: '여성 전용 사교 공간 호빠 18곳 실전 가이드. 강남 종로 영등포 시세 분위기 안전 정보 매너. 초이스 방법, 가격대 5~15만원, 첫방문 동선까지. 여자 혼자도 안전하게.' },

  // Interactive pages
  { path: '/guide', title: '처음이라 긴장된다고? 이거 읽고 가면 프로다', desc: '클럽 나이트 라운지 룸 요정 호빠 업종별 입문 가이드. 드레스코드 예산 혼자 가도 되는지 안전 팁까지. 5분이면 다 읽어요.' },
  { path: '/map', title: '지금 위치에서 가까운 곳, 지도에 다 떴다', desc: '강남 홍대 이태원 일산 부산 핫스팟 지도. 핀 클릭 한 번에 전화·길찾기·평점·시세까지 확인. 영업 중인 업소만 자동 표시.' },
  { path: '/quiz', title: '클럽형인지 라운지형인지, 테스트 해봐', desc: '10문항 2분이면 나한테 맞는 유흥 스타일 결과. 클럽 라운지 나이트 룸 요정 호빠 6가지 유형. 친구와 비교 공유 가능.' },
  { path: '/roulette', title: '고민 끝, 룰렛이 대신 골라준다', desc: '오늘 밤 갈 곳 못 정했을 때 탭 한 번이면 결정. 지역·업종·예산·인원 필터 적용해 평점 가중치 랜덤 추천. 다시 돌리기 무제한.' },
  { path: '/vs', title: '어디가 더 낫냐고? 투표로 결판내자', desc: '강남 클럽 vs 강남 클럽, 홍대 라운지 vs 이태원 라운지 등 인기 업소 맞대결. 회원 실시간 투표 결과로 진짜 1위 확인.' },
  { path: '/ranking', title: '지금 이 순간, 사람들이 가장 많이 보는 곳', desc: '실시간 조회수·평점·전화클릭 합산 TOP 30. 지역·업종별 필터로 강남 홍대 이태원 일산 부산 1위 업소 확인. 매시간 자동 업데이트.' },
  { path: '/venue-info', title: '양주·부스·룸 한눈에 보기', desc: '업종별 양주 라인업(발렌타인 조니워커 로얄살루트), 부스 사이즈, 룸 타입까지 정리. 가기 전에 미리 확인하고 예약하세요.' },
  { path: '/compare', title: '두 곳 놓고 따져보면 후회가 없다', desc: '가격·분위기·후기·평점·접근성 항목별 두 업소 비교표. 강남 vs 홍대, 클럽 vs 라운지 인기 매치 모음. 고민 끝, 선택만 남았어요.' },
  { path: '/search', title: '이름만 치면 바로 나온다, 통합 검색', desc: '지역·업종·이름·분위기·예산 아무거나 입력하면 120곳 중 딱 맞는 곳. 자동완성·오타 보정·인기 검색어 추천까지 통합 검색 엔진.' },
  { path: '/magazine', title: '밤문화 읽을거리, 여기 다 모았다', desc: '강남 홍대 이태원 일산 지역 분석, 클럽 vs 라운지 업종 비교, 신규 매장 현장 리포트. 가기 전에 읽으면 달라지는 매거진.' },
  ...
  { path: '/community/qna', title: '오늘 밤 어디 가냐고? 여기서 추천받아', desc: '갈 곳 못 정한 사람들이 모여 서로 추천해주는 Q&A 게시판. 인원·예산·분위기·지역만 적으면 단골 회원들이 빠르게 답해줍니다.' },
  { path: '/community/reviews', title: '가본 사람만 쓸 수 있다, 실제 방문 후기', desc: '별점·한 줄 평으로 보는 업소 리얼 리뷰. 광고 아닌 진짜 목소리. 클럽 나이트 라운지 룸 요정 호빠 모든 업종 후기 모음.' },
  ...
  { path: '/login', title: '카카오 탭 한 번, 3초면 로그인 끝', desc: '카카오·네이버·구글 3초 로그인. 가입 즉시 100P, 1줄 글쓰기·찜하기·후기 작성·VS 투표까지. 닉네임만 공개되고 본명은 절대 비공개.' },
  { path: '/profile', title: '내 찜 목록·후기·방문 기록 모아보기', desc: '찜한 업소·작성한 후기·포인트 내역·등급 현황·알림 설정 한 곳에. 매일 출석 +10P, 후기 +500P, 친구 초대 +1000P 적립 시스템.' },
  { path: '/dashboard', title: '내 매장 현황판, 실시간 확인', desc: '오늘 방문자·전화 클릭·찜 추가·후기 작성 수·유입 지역 분포까지. 사장님 전용 5분마다 자동 갱신 실시간 매장 현황 대시보드.' },
  { path: '/analytics', title: '유입 경로부터 전화 건수까지, 분석 리포트', desc: '어디서 들어왔고 뭘 눌렀고 전환 몇 건인지 그래프로. 요일·시간대 트래픽, 클릭률, 평점 추이까지. 매주 월요일 자동 리포트.' },
  { path: '/billing', title: '구독·결제 내역 한눈에', desc: '현재 요금제·결제 이력·변경·해지 한 페이지에서 처리. 무료/스타트/프로/VIP 4단계 요금제. 14일 무료 체험 후 자동 결제.' },
  { path: '/onboarding', title: '3분이면 끝나는 입점 신청', desc: '사업자등록증·매장 사진 3장이면 입점 완료. 24시간 내 심사·노출 시작. 14일 무료 체험 후 요금제 선택. 복잡한 거 전혀 없어요.' },
  { path: '/launch', title: '심사 통과! 오픈 전 마지막 체크', desc: '대시보드 접속 전 사진·영업시간·메뉴·알림 설정·요금제 체크리스트. 첫 일주일 노출량이 검색 순위 결정하니 꼼꼼하게.' },
  { path: '/admin/venues', title: '매장 수정·삭제', desc: '관리자 전용 페이지. 업소 정보 수정·사진 교체·영업 상태 토글·리뷰 신고 처리·광고 노출 영역 변경 모두 가능.' },
];
