...
function generateVenueSsrBody(v, allVenues) {
  ...
  {
    const hubCm = catMap[v.cat];
    if (hubCm) {
      html += `<p><a href="/region/${encodeURIComponent(v.regionKo)}/${hubCm.path}/">${escHtml(lastRegion)} ${catKo} ${pickN(['전체 목록', '다른 곳까지 한눈에', '모아보기', '전부 비교하기'], 35)}</a></p>`;
    }
  }
  html += `</section>`;
...
const staticPages = [
  ...
  { path: '/clubs', title: '클럽 줄 서다 입장컷 당하기 싫죠? 10년 MD가 들어갈 곳만 알려줌', desc: '클럽 줄 컷이면 그날 끝. 10년 MD가 강남·홍대 클럽 Funktion-One·드레스코드·해외 게스트 DJ·새벽 3시 피크·VIP 부킹까지 클럽 121곳, 갈 곳만 추렸으니 줄 서기 전에 바로 확인 →' },
  ...
  { path: '/guide/ilsan-yojeong', title: '일산요정 첫 방문 가이드 — 전통 한정식 정찬과 국악 접객, 예약까지 한눈에', desc: '일산요정 첫 방문이라 막막하다면? 전통 한정식 15첩 정찬과 거문고·해금 국악, 한복 접객의 격식부터 마두역 인근 위치와 예약·주차까지, 일산명월관요정 실정보 기준으로 담백하게 짚었다.' },
  ...
  { path: '/cafe', title: '네이버 카페 두 곳 — 1관 정보와 2관 소식, 어디로 갈까', desc: '놀쿨 카페는 1관과 2관으로 나뉩니다. 1관은 처음 가는 분을 위한 위치·영업시간·복장 정보, 2관은 다니는 분을 위한 이벤트·행사 소식. 만 19세 이상.' },
  ...
  // Legal & Info
  { path: '/privacy', title: '개인정보 어디까지 안전한가 — 수집·이용·파기 안내', desc: '내 개인정보 어디까지 안전할까? 수집 항목·보유 기간·제3자 제공 여부를 투명하게 공개합니다. 개인정보보호법 준수, SSL 암호화, 자동 파기, 회원 탈퇴 시 즉시 삭제까지 가입 전 먼저 확인하세요.' },
  { path: '/terms', title: '이용약관 전문 — 가입·활동·탈퇴 시 적용되는 회원 권리다', desc: '가입하면 내 권리는 어디까지일까? 놀쿨 회원 가입·이용·탈퇴에 적용되는 권리와 의무 전문입니다. 운영 정책, 금지 행위, 저작권, 분쟁 해결, 약관 변경 절차까지 가입 전 먼저 살펴보세요.' },
  ...
  { path: '/venue-terms', title: '업소 등록 전 꼭 봐야 할 광고 게재 약관이다', desc: '내 매장 광고, 어떤 조건으로 게재될까? 게재 조건·환불 정책·삭제 기준을 명시했습니다. 신청 절차, 콘텐츠 검수, 부적절 광고 처리, 결제·환불·해지, 분쟁 중재까지 사장님 등록 전 먼저 확인하세요.' },
  ...
  { path: '/legal', title: '법적 준수 안내 — 정보통신망법·청소년보호법·개인정보보호법 한 번에 정리했다', desc: '우리가 지키는 법은 어디까지일까? 본 플랫폼이 준수하는 국내 법령과 운영 원칙을 한 페이지에 정리했습니다. 청소년 보호, 콘텐츠 검수, 개인정보 처리, 신고 창구까지 운영 책임 범위를 먼저 확인하세요.' },
  ...
  { path: '/pricing', title: '요금제 안내 — 매장 규모별 4단계 선택할 수 있다', desc: '내 매장엔 어떤 요금제가 맞을까? 매장 규모와 노출 필요도에 맞춰 단계별 요금제를 고를 수 있습니다. 결제·환불·법인 세금계산서 등 자세한 사항은 약관·결제 문서에서 먼저 확인하세요.' },
  ...
  // Lead magnets
  { path: '/lead/nightlife-guide', title: '서울경기 나이트라이프 — 업종별로 등록 업소 바로 찾기', desc: '어디부터 봐야 할지 모르겠다면? 클럽·나이트·라운지·룸·요정·호빠 6개 카테고리에서 서울·경기 등록 업소를 지역별로 바로 확인하세요. 새 큐레이션 소식도 이메일로 받아볼 수 있습니다.' },
  ...
  // Misc
  { path: '/status', title: '지금 서비스 상태 — 점검·장애·복구 실시간 공지한다', desc: '지금 놀쿨 잘 돌아가고 있을까? 현재 서비스 상태와 예정된 점검 일정을 안내합니다. 검증된 측정값이 확보된 후에만 구체 수치를 게시하며, 장애·복구 이력은 시간순으로 먼저 확인할 수 있습니다.' },
  ...
  { path: '/login', title: '카카오·구글 1분이면 로그인 끝난다', desc: '회원 안 하면 찜·후기 어디 가? OAuth 1번 로그인이면 끝. 본명·주민번호 입력 없이 닉네임만으로 바로 활동.' },
  ...
  { path: '/onboarding', title: '입점 신청 안내 — 사장님 5분이면 등록 끝난다', desc: '사장님, 입점은 어떻게 시작할까? 사업자등록증과 매장 사진만 등록하면 신청이 가능합니다. 정확한 1차 정보 등록을 권장하며, 잘못된 정보는 노출에 영향을 줄 수 있으니 먼저 확인하세요.' },
...
function _parseEarlyMagazineArticles() {
  ...
  for (let i = 1; i < blocks.length; i++) {
    ...
    if (id && title) result.push({ id, title, excerpt: excerpt || '' });
  }
  return result;
}
...
const INTERACTIVE_PAGE_BLURBS = {
  '/cafe': () => `<h1>놀쿨 카페는 두 곳입니다</h1><p>1관은 처음 가는 분을 위한 정보, 2관은 다니는 분을 위한 소식입니다. 만 19세 이상만 이용합니다.</p><h2>1관 정보 — 전국 나이트클럽 정보·후기</h2><p><a href="https://cafe.naver.com/qotjsdnr?utm_source=nolcool&amp;utm_medium=cafe_page&amp;utm_campaign=hall1" target="_blank" rel="noopener">1관 정보 카페로 가기</a> — 위치·영업시간·주차·복장·가는 길을 모아 둔 곳입니다.</p><h2>2관 소식 — 전국 나이트 이벤트·소식</h2><p><a href="https://cafe.naver.com/beasunwook?utm_source=nolcool&amp;utm_medium=cafe_page&amp;utm_campaign=hall2" target="_blank" rel="noopener">2관 소식 카페로 가기</a> — 이벤트·행사·시간대 팁을 올리는 곳입니다.</p><h2>가입 3단계</h2><ol><li>카페 선택 — 처음이면 1관, 다니는 분이면 2관</li><li>가입 — 네이버 로그인 후 만 19세 확인</li><li>가입인사 한 줄 — 지역·관심 업소 한 줄이면 됩니다</li></ol><h2>등업 혜택</h2><table><thead><tr><th>등급</th><th>할 수 있는 것</th></tr></thead><tbody><tr><td>새싹</td><td>가입 즉시 글·질문</td></tr><tr><td>단골</td><td>가입 3일·방문 5·글 1·댓글 5 자동 등업 → 가이드·교통 게시판 글쓰기</td></tr><tr><td>우수·VIP</td><td>댓글왕·후기왕 후보</td></tr></tbody></table><h2>어느 카페로 갈지 한눈에</h2><table><thead><tr><th>항목</th><th>내용</th></tr></thead><tbody><tr><td>1관</td><td>위치·영업시간·주차·복장·가는 길</td></tr><tr><td>2관</td><td>이벤트·행사·시간대 팁</td></tr><tr><td>공통</td><td>만 19세 이상 · 운영 놀쿨(nolcool.com) · 광고·제휴 문의 카카오톡 besta12</td></tr></tbody></table><p>가짜 후기·가짜 회원은 만들지 않습니다. 운영자 글은 놀쿨 이름으로만 올립니다.</p>`,
  ...
  '/login': () => `<h1>카카오 한 번에 로그인</h1><p>가입하면 후기 작성, 찜하기, 1줄 글쓰기, VS 투표 전부 가능합니다. 카카오·구글 계정으로 탭 한 번이면 됩니다(필수 동의 3개 · 선택 알림은 꺼진 채로 시작).</p><h2>로그인하면 뭐가 달라지나</h2><ul><li>1줄 글쓰기 — 한 줄로 다녀온 곳 자랑</li><li>찜하기 — 가고 싶은 곳 모아두기</li><li>후기 작성</li><li>VS 투표 참여</li><li>퀴즈·룰렛 결과 저장</li></ul><h2>가입 안내</h2><ul><li>가입은 무료, 본명·주민번호 입력 없음</li><li>닉네임만 정하면 끝</li><li>탈퇴 1클릭 가능</li></ul><h2>개인정보 보호</h2><p>닉네임만 공개되고 본명·연락처는 공개되지 않습니다. 채팅·댓글에서도 익명이 보장되며, 탈퇴 시 모든 데이터가 즉시 삭제됩니다.</p>`,
  ...
  '/magazine': () => {
    const articleLis = _earlyMagazineList.map(a => a.excerpt
      ? `<li><a href="/magazine/${a.id}/">${escHtml(a.title)}</a> — ${escHtml(a.excerpt)}</li>`
      : `<li><a href="/magazine/${a.id}/">${escHtml(a.title)}</a></li>`).join('');
    return `<h1>나이트라이프 매거진</h1><p>가기 전에 읽으면 헛걸음 안 하는 글만 모았습니다. 지역별로 어디가 정통이고 어디가 부킹 잘 되는지, 같은 업종이라도 분위기가 어떻게 갈리는지 — 직접 다녀온 사람 시선으로 정리했습니다. 검색해서 들어온 김에 한 편만 읽어봐도 오늘 밤 동선이 달라집니다.</p><h2>최신 매거진 글 ${_earlyMagazineList.length}편</h2><ul>${articleLis}</ul><h2>권역 가이드부터 읽으면 좋은 이유</h2><p>같은 "나이트"라도 서면 정통이냐 연산동 부킹이냐, 둔산동 격식이냐 봉명동 온천 부킹이냐에 따라 입구에서부터 분위기가 갈립니다. 권역 가이드는 한 지역 안에서 성격이 다른 세 곳을 나란히 놓고 "이런 날엔 여기, 저런 날엔 저기"를 골라주는 글입니다. 처음 가는 동네일수록 헛걸음 확률을 크게 줄여줍니다.</p><h2>매거진 카테고리</h2><ul><li>권역 가이드 — 부산·대전·인천·성남·동북권 등 한 지역 3~4곳 비교</li><li>업종 비교 — 클럽 vs 라운지, 룸 vs 요정 콘셉트 차이</li><li>장르 분석 — 이태원 라틴·힙합·트랜스·언더처럼 음악으로 갈라보기</li><li>현장 리포트 — 신규 오픈·재단장 매장 첫 방문기</li><li>트렌드 — 시즌 이벤트·DJ 게스트·음악 흐름</li></ul><h2>이렇게 활용하세요</h2><p>먼저 가려는 지역 권역 가이드를 펼쳐 세 곳의 성격을 비교하고, 마음에 드는 곳의 업소 상세 페이지에서 양주 라인업·부스 구성·영업시간을 확인하면 됩니다. 같은 동선에 묶기 좋은 호핑 코스도 글 안에 정리해뒀으니 2차·3차까지 미리 그려볼 수 있습니다.</p><h2>매거진 이용 안내</h2><p>새 글이 등록되면 회원 알림 설정에 따라 안내됩니다. 좋아요·댓글이 활발한 글은 홈 영역에 노출될 수 있습니다.</p>`;
  },
...
for (const pg of staticPages) {
  ...
  if (categoryPaths.has(pg.path)) {
    ...
    const CAT_FAQ_EXTRA = {
      ...
      hoppa: [
        { q: `호빠 여자 혼자 방문해도 안전해요?`, a: `1인 방문도 많습니다. 모든 등록 업소는 영업 확인을 거쳤고, 실장이 처음부터 끝까지 안내합니다. 불쾌한 상황 시 즉시 직원에게 요청 가능합니다.` },
...
const CROSS_SIG = {
  night: {
    ...
    '대전':   { tail: '둔산동 트로트 라이브부터 중앙로 38세+ 홀까지', sig: '둔산동 트로트 라이브부터 중앙로 38세+ 편안한 홀까지 한자리에서 비교' },
  ...
  hoppa: {
    ...
    // '해운대' 단독 키 제거(2026-08-02): venue regionKo는 '부산 해운대'만 존재 → 이 키는 어떤 페이지에도
    // 매칭된 적 없는 죽은 설정. watch가 이걸 복사해 매일 404 거짓경보를 냈으므로 양쪽 동기 삭제.
...
for (const v of venues) {
  ...
  // 시즌180 — 공용 훅 prefix 폐기: 카테고리 공용 3문구가 venue 수십 곳 desc 머리에 반복돼
  // SERP 스니펫 첫 20자가 전부 동일(n-gram 5회 초과 + 순위 대비 저CTR 실측). venue 고유
  // shortDesc(answer-first)가 곧 후킹 — React(useDocumentMeta) desc와도 동일해진다.
...
<ul>${_relLis}<li><a href="/clubs/">클럽 큐레이션 전체</a></li><li><a href="/nights/">나이트 큐레이션 전체</a></li></ul>
  ...
  // ★ 시즌159 'articleAddon' 패딩 제거 (2026-06-16): tag/title만 바뀌는 6개 H2 보일러플레이트가
  //   전 매거진 글에 공통 삽입 → 5-gram Jaccard 평균 33.9%/최대 58.2%의 구조 지문(scaled-content-abuse)
...
// ── 집계 페이지(tag/region/near/best/new) 구조지문 해체 + 허브-메시 cross-link 헬퍼 ──
// 시즌88 — tag/near/new/best/region 본문이 동일 템플릿 문단 반복(5gram Jaccard 84~96%) → Google "Discovered/Crawled - not indexed".
// 해결: 멤버 업소 실데이터로 고유 본문 구성 + 페이지마다 다른 허브-메시 cross-link(막다른길 0). 연결어는 key 해시로 회전.
function aggHash(key) { let h = 0; for (let i = 0; i < key.length; i++) h = (h * 31 + key.charCodeAt(i)) >>> 0; return h; }
function aggPick(key, arr, off) {
  let x = (aggHash(key) ^ Math.imul((off || 0) + 1, 0x9e3779b9)) >>> 0;
  x = Math.imul(x ^ (x >>> 16), 0x45d9f3b) >>> 0;
  x = Math.imul(x ^ (x >>> 16), 0x45d9f3b) >>> 0;
  x = (x ^ (x >>> 16)) >>> 0;
  return arr[x % arr.length];
...
function aggHubMesh(members, selfKind, selfKey) {
  ...
  const cats = [...new Set(members.map(v => v.cat))].filter(c => !((selfKind === 'best' || selfKind === 'new') && c === selfKey));
  let html = '';
  if (topTags.length) {
    html += `<h2>함께 찾는 키워드</h2><ul>`;
    for (const t of topTags) html += `<li><a href="/tag/${encodeURIComponent(t)}/">#${escHtml(t)} 모아보기</a></li>`;
    html += `</ul>`;
  }
  if (regions.length) {
  ...
  if (stations.length) {
    html += `<h2>역 근처로 둘러보기</h2><ul>`;
    for (const s of stations) html += `<li><a href="/near/${encodeURIComponent(s)}/">${escHtml(s)} 근처 업소</a></li>`;
    html += `</ul>`;
  }
  return html;
}
// 인라인 CTA — 다크패턴 아님(공격적 팝업 X). 다음 행동 안내만. key로 문구 회전.
function aggInlineCta(key, members) {
  const v = members[0];
  const verb = aggPick(key, ['마음에 드는 곳을 찜해 두고', '관심 가는 곳을 저장해 두고', '가고 싶은 곳을 찜해 두면'], 9);
  const join = aggPick(key, ['커뮤니티에서 실제 다녀온 후기를 비교해 보세요', '커뮤니티 후기로 분위기를 한 번 더 확인해 보세요', '커뮤니티에서 회원 후기를 참고하면 결정이 쉽습니다'], 10);
  return `<p class="agg-cta">${verb} <a href="/community/reviews/">${join}</a>. 바로 가고 싶다면 ${v ? `<a href="${venueHref(v)}">${escHtml(v.nameKo)}</a> 상세에서 직통 번호로 예약 가능 시간을 확인하면 됩니다.` : '각 업소 상세에서 직통 번호로 예약 가능 시간을 확인하면 됩니다.'}</p>`;
}
...
for (const [catKey, catInfo] of Object.entries(catMap)) {
  ...
  // 시즌88 — 동일 템플릿 문단 제거 → 멤버 실데이터 본문 + 허브-메시(막다른길 0)
  const newShort = (vv) => { const parts = (vv.nameKo || '').split(/\s+/); return parts.length > 1 ? parts.slice(1).join(' ') : vv.nameKo; };
  const newRegionList = [...new Set(catVenues.map(vv => vv.regionKo))].slice(0, 8);
  const newRegionStr = newRegionList.slice(0, 4).map(escHtml).join(', ');
  let ssrBody = `<h1>${escHtml(title)}</h1><p>${escHtml(desc)}</p>`;
  ssrBody += `<p>${aggPick('new-' + catKey, ['최근', '요즘', '이번에'], 0)} 새로 입점한 ${escHtml(catInfo.labelKo)} ${catVenues.length}곳은 ${newRegionStr}${newRegionList.length > 4 ? ' 등' : ''} 지역에 ${aggPick('new-' + catKey, ['새로 생겼습니다', '문을 열었습니다', '오픈했습니다'], 1)}. 단골이 자리 잡기 전이라 응대·자리가 가장 여유로운 시기입니다.</p>`;
...
for (const [tag, tagVenues] of Object.entries(allTags)) {
  ...
  const tagCatStr = tagCats.map(escHtml).join('·');
  // desc도 실제 멤버 업종·지역만 노출(전체 6업종/고정 지역 나열 X) → 태그마다 고유
  const desc = `'${tag}' ${aggPick(tag, ['키워드로 묶인', '결로 모은', '태그가 붙은'], 11)} ${tagCats.join('·')} ${tagVenues.length}곳, ${aggPick(tag, ['오늘 밤 어디부터 가볼까', '어느 곳부터 가볼지 고민이라면', '어디부터 둘러볼까'], 13)}? ${tagTopNames} 등 ${tagRegions.slice(0, 3).join(', ')}${tagRegions.length > 3 ? ' 등' : ''} 업소를 ${aggPick(tag, ['같은 결끼리 모아 한눈에 비교합니다', '한 페이지에서 바로 비교하세요', '분위기·후기·전화번호까지 정리했습니다'], 12)}.`;
  let ssrBody = `<h1>${escHtml(title)}</h1><p>${escHtml(desc)}</p>`;
