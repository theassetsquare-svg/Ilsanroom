...
const staticPages = [
  ...
  { path: '/yojeong', title: '요정 갔는데 격 떨어지면 끝장이죠? 20년 실장이 봐드림', desc: '사장님 모시는데 격 떨어지면 다음은 없어요. 20년 일한 요정 실장이 한정식·접대·진행 매너 한 줄로 정리합니다. 진짜 격 있는 곳만 →' },
  { path: '/hoppa', title: '호빠 가서 사진이랑 다른 선수면 어떡해요? 10년 실장이 미리 봐줌', desc: '처음 가서 어색한 시간 30분이면 그날 끝나요. 10년 일한 호빠 실장이 외모·매너·진행 다 봐드립니다. 진짜 케어되는 곳만 보여드릴게요 →' },
  ...
  { path: '/vs', title: '어디가 더 낫냐고? 투표로 결판내자', desc: '인기 업소 맞대결을 회원 투표로 비교. 회원이 직접 매치업을 제안하고, 결과 댓글에서 선택 이유를 토론합니다.' },
  ...
  { path: '/testimonials', title: '업주 인터뷰 모음', desc: '검증된 업주 인터뷰만 게시. 가공된 인용·과장된 수치 없이 발화 원문 그대로 공개. 인터뷰 참여를 원하는 업주는 운영자에게 문의해주세요.' },
  { path: '/pricing', title: '요금제 안내 — 매장 규모별 선택', desc: '매장 규모와 노출 필요도에 따라 단계별 요금제 선택. 결제·환불·법인 세금계산서 등 자세한 사항은 약관·결제 문서에서 확인하세요.' },
  { path: '/demo', title: '업주 대시보드 미리보기', desc: '가입 전 사장님 대시보드 구성을 미리 확인. 리뷰 관리, 사진 업로드, 영업 상태 변경, 알림 설정 등 화면을 둘러볼 수 있는 데모 페이지.' },
  { path: '/case-studies', title: '운영 사례 안내', desc: '매장 운영을 잘하는 곳들이 공통적으로 챙기는 항목 정리. 검증되지 않은 매출·증가율 수치는 게시하지 않습니다.' },
  ...
  { path: '/lead/weekly-hot', title: '이번 주 핫플 큐레이션 — 준비 중', desc: '커뮤니티 후기와 검색 추이를 종합한 주간 핫플 큐레이션은 데이터 파이프라인이 연결되는 대로 공개됩니다. 발송 일정은 미리 약속하지 않습니다.' },

  // Misc
  { path: '/status', title: '서버 상태·점검 일정', desc: '현재 서비스 상태와 예정된 점검 일정 안내. 검증된 측정값이 확보된 후에만 가동률 등 구체 수치를 게시합니다.' },
  { path: '/referral', title: '친구 초대 안내', desc: '친구 초대 링크 공유 안내 페이지. 적립·등급 정책은 운영 정책 문서를 참고하세요. 부정 가입 행위는 자동 차단됩니다.' },
  { path: '/hidden', title: '덜 알려진 업소 모음', desc: '광고보다 입소문으로 운영되는 업소 모음. 등록 여부는 업소 협조에 따라 달라지며, 추천이 있으면 커뮤니티로 제보해주세요.' },
  { path: '/gallery', title: '매장 내부 사진 갤러리', desc: '업주·회원이 등록한 매장 내부 사진 모음. 조명·룸 배치·무대 분위기·라인업까지 가기 전에 눈으로 확인하세요.' },
  { path: '/events', title: '업소 이벤트·행사 일정', desc: 'DJ 게스트, 시즌 이벤트 등 업소가 직접 등록한 일정 모음. 정확한 정보는 매장 페이지에서 다시 확인해주세요.' },
...
const INTERACTIVE_PAGE_BLURBS = {
  ...
  '/dashboard': () => `<h1>내 매장 현황판</h1><p>등록 업소의 페이지뷰·전화 클릭·찜·후기 지표를 사장님 전용으로 확인할 수 있는 페이지입니다.</p><h2>대시보드 주요 지표</h2><ul><li>페이지뷰</li><li>전화번호 클릭 수</li><li>찜하기 추가 수</li><li>후기 작성 수</li><li>유입 지역 분포</li></ul><h2>매장 운영 인사이트</h2><ul><li>요일별 방문자 추이</li><li>시간대별 클릭 분포</li><li>리뷰 평점 추이</li></ul><h2>대시보드 활용 팁</h2><p>지표가 정체되면 사진·메뉴를 업데이트해보세요. 부정 후기에는 24시간 내 답변을 권장합니다. 데이터는 모바일에서도 동일하게 확인할 수 있습니다.</p>`,
  ...
  '/case-studies': () => `<h1>운영 사례 안내</h1><p>입점 후 운영을 잘하는 매장들이 공통적으로 챙기는 항목을 정리했습니다. 구체적 매출·증가율 등 검증되지 않은 수치는 게시하지 않습니다.</p><h2>운영을 잘하는 매장의 공통 행동</h2><ul><li>사진을 충분히 등록 (가급적 최근 촬영본)</li><li>리뷰에 24시간 내 답변</li><li>대표 메뉴·라인업 정확히 기입</li><li>영업시간·연락처 최신 상태 유지</li><li>고객 응대 매뉴얼 일관성</li></ul><h2>활용법</h2><p>자기 매장의 현재 지표는 대시보드에서 직접 확인할 수 있습니다. 운영 컨설팅이 필요하면 업주 상담을 신청해주세요.</p>`,
