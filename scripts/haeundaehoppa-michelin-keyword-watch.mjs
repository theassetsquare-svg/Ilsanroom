/**
 * 해운대호빠 미슐랭 페이지 2 키워드 SEO 24h watch (시즌125).
 * 매일 KST 11:35 — title/desc/body 검색 의도 매칭 측정 → 회귀시만 메일.
 *
 * 측정 키워드:
 *   ① "해운대호빠" — venue 지역+업종 (PRIMARY, 사용자 1순위 명시)
 *   ② "부산호빠"  — 광역 흡수 (SECONDARY, 사용자 명시, 부산 단골 → 해운대 마지막 코스 동선)
 *
 * 12지표 + 디테일 토큰:
 *   1) HTTP 200
 *   2) title에 "해운대호빠" 포함
 ...
 *   4) title ≤60자
 *   5) title 중복단어 없음
 *   6) desc 150자 이내 + "해운대호빠" 포함
 *   7) desc에 "부산호빠" ≥3회
 *   8) "해운대호빠" 밀도 ≤3.5%
 *   9) "부산호빠" body 등장 ≥3회
 *  10) 후킹 5축 ≥1축 (title 또는 desc)
 *  11) 디테일 토큰 (해운대역·동백섬·마린시티·더베이101·달맞이고개) 모두 등장
 *     — 해운대 5대 시그니처 lock, 해운대호빠 1순위 + 부산호빠 광역 동시 의도
 *  12) "해운대호빠" body 등장 ≥5회
 *
 * 환경:
 *   RESEND_API_KEY     필수
...
const URL = 'https://nolcool.com/hoppa/haeundaehoppa-michelin/';
const PRIMARY = '해운대호빠';
const SECONDARY = '부산호빠';
const DETAIL_TOKENS = ['해운대역', '동백섬', '마린시티', '더베이101', '달맞이고개'];

function fetchHtml(url) {
  return new Promise((res) => {
    const t = setTimeout(() => res({ status: 0, html: '' }), 20000);
    https.get(url, { headers: { 'User-Agent': 'NolcoolMichelinWatch/1.0' } }, r => {
      const chunks = [];
...
async function main() {
  ...
  const secondaryInDesc = (desc.match(new RegExp(SECONDARY, 'g')) || []).length;
  ...
  if (!title.includes(PRIMARY)) issues.push(`title 해운대호빠X`);
  if (!absorbsSecondary(title, SECONDARY)) issues.push(`title 부산호빠X`);
  ...
  if (!desc.includes(PRIMARY)) issues.push(`desc 해운대호빠X`);
  if (desc.length === 0 || desc.length > 150) issues.push(`desc ${desc.length}자`);
  if (secondaryInDesc < 3) issues.push(`desc 부산호빠 ${secondaryInDesc}회 (≥3 필요)`);
  if (primaryDensity > 0.035) issues.push(`해운대호빠 밀도 ${(primaryDensity*100).toFixed(2)}%`);
  if (primaryCount < 5) issues.push(`해운대호빠 body ${primaryCount}회 (≥5 필요)`);
  if (secondaryCount < 3) issues.push(`부산호빠 body ${secondaryCount}회 (≥3 필요)`);
  ...
  const snapshot = {
    title, desc, primaryCount, secondaryCount, secondaryInDesc,
  ...
  console.log('해운대호빠 미슐랭 SEO watch');
  console.log('  URL:', URL);
  console.log('  title:', title);
  console.log('  desc:', desc);
  console.log('  해운대호빠:', primaryCount, '회 / 밀도', snapshot.primaryDensity);
  console.log('  부산호빠:', secondaryCount, '회 (desc', secondaryInDesc, '회)');
...
async function sendMail({ issues, snapshot }) {
  ...
  const html = `<div style="font-family:sans-serif;max-width:680px;margin:0 auto;padding:20px">
    <h2 style="color:#DC2626">[⚠ 해운대호빠 미슐랭 SEO 회귀] ${issues.length}건</h2>
    ...
    <table style="border-collapse:collapse;width:100%;font-size:12px">
      ...
      <tr><td style="border:1px solid #E5E7EB;padding:6px">해운대호빠</td><td style="border:1px solid #E5E7EB;padding:6px">${snapshot.primaryCount}회 / 밀도 ${esc(snapshot.primaryDensity || '')}</td></tr>
      <tr><td style="border:1px solid #E5E7EB;padding:6px">부산호빠</td><td style="border:1px solid #E5E7EB;padding:6px">${snapshot.secondaryCount}회 (desc ${snapshot.secondaryInDesc}회)</td></tr>
    ...
    <p style="color:#9CA3AF;font-size:11px;margin-top:20px">매일 KST 11:35 자동 — haeundaehoppa-michelin-keyword-watch.mjs</p>
  </div>`;
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [TO],
      subject: `[놀쿨][⚠] 해운대호빠 미슐랭 SEO 회귀 ${issues.length}건`,
      html,
    }),
  });
