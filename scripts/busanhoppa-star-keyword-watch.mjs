/**
 * 부산호빠 스타 페이지 2 키워드 SEO 24h watch (시즌118).
 * 매일 KST 11:00 — title/desc/body 검색 의도 매칭 측정 → 회귀시만 메일.
 *
 * 측정 키워드:
 *   ① "부산호빠"   — venue 지역+업종 (PRIMARY)
 *   ② "해운대호빠" — 인접 광역 (SECONDARY, 사용자 직접 명시, 광안리→서면 차로 25분 코스 동선)
 *
 * 10지표 + 디테일 토큰:
 *   1) HTTP 200
 *   2) title에 "부산호빠" 포함
 ...
 *  +) 디테일 토큰 (쥬디스태화/전포카페거리/서면역/블랙마블/케이크세리머니) 모두 등장
 *
 * 환경:
 *   RESEND_API_KEY     필수
...
const URL = 'https://nolcool.com/hoppa/busanhoppa-star/';
const PRIMARY = '부산호빠';
const SECONDARY = '해운대호빠';
const DETAIL_TOKENS = ['쥬디스태화', '전포카페거리', '서면역', '블랙마블', '케이크세리머니'];
...
function fetchHtml(url) {
  ...
  const _once = () => new Promise((res) => {
      ...
      https.get(url, { headers: { 'User-Agent': 'NolcoolBusanStarWatch/1.0' } }, r => {
        const chunks = [];
...
async function main() {
  ...
  if (!title.includes(PRIMARY)) issues.push(`title 부산호빠X`);
  if (!absorbsSecondary(title, SECONDARY)) issues.push(`title 해운대호빠X`);
  ...
  console.log('부산호빠 스타 SEO watch');
  console.log('  URL:', URL);
  console.log('  title:', title);
  console.log('  desc:', desc);
  console.log('  부산호빠:', primaryCount, '회 / 밀도', snapshot.primaryDensity);
  console.log('  해운대호빠:', secondaryCount, '회 (desc', secondaryInDesc, '회)');
...
async function sendMail({ issues, snapshot }) {
  ...
  const html = `<div style="font-family:sans-serif;max-width:680px;margin:0 auto;padding:20px">
    <h2 style="color:#DC2626">[⚠ 부산호빠 스타 SEO 회귀] ${issues.length}건</h2>
    ...
    <table style="border-collapse:collapse;width:100%;font-size:12px">
      ...
      <tr><td style="border:1px solid #E5E7EB;padding:6px">부산호빠</td><td style="border:1px solid #E5E7EB;padding:6px">${snapshot.primaryCount}회 / 밀도 ${esc(snapshot.primaryDensity || '')}</td></tr>
      <tr><td style="border:1px solid #E5E7EB;padding:6px">해운대호빠</td><td style="border:1px solid #E5E7EB;padding:6px">${snapshot.secondaryCount}회 (desc ${snapshot.secondaryInDesc}회)</td></tr>
    ...
    <p style="color:#9CA3AF;font-size:11px;margin-top:20px">매일 KST 11:00 자동 — busanhoppa-star-keyword-watch.mjs</p>
  </div>`;
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [TO],
      subject: `[놀쿨][⚠] 부산호빠 스타 SEO 회귀 ${issues.length}건`,
      html,
    }),
  });
