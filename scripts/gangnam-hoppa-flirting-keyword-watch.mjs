/**
 * 강남호빠 플러팅 페이지 2 키워드 SEO 24h watch (시즌113).
 * 매일 KST 10:20 — title/desc/h1/body 검색 의도 매칭 측정 → 회귀시만 메일.
 *
 * 측정 키워드:
 *   ① "강남호빠" — venue 지역+업종 (PRIMARY, 전국 최고 검색량 hoppa 키워드)
 ...
 *   4) title ≤60자
 *   5) title 중복단어 없음
 *   6) desc 150자 이내 + "강남호빠" 포함
 *   7) desc에 "서울호빠" ≥3회
 *   8) "강남호빠" 밀도 ≤3.5%
 *   9) "서울호빠" body 등장 ≥3회
 *  10) 후킹 5축 ≥1축 (title 또는 desc)
 *  +) 디테일 토큰 (역삼동/한실장/부스/콩나물국밥/조명) 모두 등장
 *
 * 환경:
 *   RESEND_API_KEY     필수
...
const URL = 'https://nolcool.com/hoppa/gangnam-hoppa-flirting/';
const PRIMARY = '강남호빠';
const SECONDARY = '서울호빠';
const DETAIL_TOKENS = ['역삼동', '한실장', '부스', '콩나물국밥', '조명'];

function fetchHtml(url) {
  return new Promise((res) => {
    const t = setTimeout(() => res({ status: 0, html: '' }), 20000);
    https.get(url, { headers: { 'User-Agent': 'NolcoolGangnamFlirtingWatch/1.0' } }, r => {
      const chunks = [];
...
async function main() {
  ...
  console.log('강남호빠 플러팅 SEO watch');
  console.log('  URL:', URL);
  console.log('  title:', title);
  console.log('  desc:', desc);
  console.log('  강남호빠:', primaryCount, '회 / 밀도', snapshot.primaryDensity);
...
async function sendMail({ issues, snapshot }) {
  ...
  const html = `<div style="font-family:sans-serif;max-width:680px;margin:0 auto;padding:20px">
    <h2 style="color:#DC2626">[⚠ 강남호빠 플러팅 SEO 회귀] ${issues.length}건</h2>
    ...
    <table style="border-collapse:collapse;width:100%;font-size:12px">
      ...
      <tr><td style="border:1px solid #E5E7EB;padding:6px">강남호빠</td><td style="border:1px solid #E5E7EB;padding:6px">${snapshot.primaryCount}회 / 밀도 ${esc(snapshot.primaryDensity || '')}</td></tr>
    ...
    <p style="color:#9CA3AF;font-size:11px;margin-top:20px">매일 KST 10:20 자동 — gangnam-hoppa-flirting-keyword-watch.mjs</p>
  </div>`;
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [TO],
      subject: `[놀쿨][⚠] 강남호빠 플러팅 SEO 회귀 ${issues.length}건`,
      html,
    }),
  });
