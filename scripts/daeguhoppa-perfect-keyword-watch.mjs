/**
 * 대구호빠 퍼펙트 페이지 2 키워드 SEO 24h watch (시즌115).
 * 매일 KST 10:30 — title/desc/h1/body 검색 의도 매칭 측정 → 회귀시만 메일.
 *
 * 측정 키워드:
 *   ① "대구호빠" — venue 지역+업종 (PRIMARY)
 *   ② "동성로호빠" — 대구 #1 유흥상권 (SECONDARY, shortDesc/description 수동 주입)
 *
 * 10지표 + 디테일 토큰:
 *   1) HTTP 200
 *   2) title에 "대구호빠" 포함
 ...
 *   4) title ≤60자
 *   5) title 중복단어 없음
 *   6) desc 150자 이내 + "대구호빠" 포함
 *   7) desc에 "동성로호빠" ≥3회
 *   8) "대구호빠" 밀도 ≤3.5%
 *   9) "동성로호빠" body 등장 ≥3회
 *  10) 후킹 5축 ≥1축 (title 또는 desc)
 *  +) 디테일 토큰 (엘리에나호텔/반월당역/70개/올초이스/발렛) 모두 등장
 *
 * 환경:
 *   RESEND_API_KEY     필수
...
const DETAIL_TOKENS = ['엘리에나호텔', '반월당역', '70개', '올초이스', '발렛'];
...
function fetchHtml(url) {
  ...
  const _once = () => new Promise((res) => {
      ...
      https.get(url, { headers: { 'User-Agent': 'NolcoolDaeguPerfectWatch/1.0' } }, r => {
        const chunks = [];
...
async function main() {
  ...
  if (!title.includes(PRIMARY)) issues.push(`title 대구호빠X`);
  if (!absorbsSecondary(title, SECONDARY)) issues.push(`title 동성로호빠X`);
  ...
  if (!desc.includes(PRIMARY)) issues.push(`desc 대구호빠X`);
  if (desc.length === 0 || desc.length > 150) issues.push(`desc ${desc.length}자`);
  if (secondaryInDesc < 3) issues.push(`desc 동성로호빠 ${secondaryInDesc}회 (≥3 필요)`);
  if (primaryDensity > 0.035) issues.push(`대구호빠 밀도 ${(primaryDensity*100).toFixed(2)}%`);
  if (secondaryCount < 3) issues.push(`동성로호빠 body ${secondaryCount}회 (≥3 필요)`);
  ...
  console.log('대구호빠 퍼펙트 SEO watch');
  console.log('  URL:', URL);
  console.log('  title:', title);
  console.log('  desc:', desc);
  console.log('  대구호빠:', primaryCount, '회 / 밀도', snapshot.primaryDensity);
  console.log('  동성로호빠:', secondaryCount, '회 (desc', secondaryInDesc, '회)');
...
async function sendMail({ issues, snapshot }) {
  ...
  const html = `<div style="font-family:sans-serif;max-width:680px;margin:0 auto;padding:20px">
    <h2 style="color:#DC2626">[⚠ 대구호빠 퍼펙트 SEO 회귀] ${issues.length}건</h2>
    ...
    <table style="border-collapse:collapse;width:100%;font-size:12px">
      ...
      <tr><td style="border:1px solid #E5E7EB;padding:6px">대구호빠</td><td style="border:1px solid #E5E7EB;padding:6px">${snapshot.primaryCount}회 / 밀도 ${esc(snapshot.primaryDensity || '')}</td></tr>
      <tr><td style="border:1px solid #E5E7EB;padding:6px">동성로호빠</td><td style="border:1px solid #E5E7EB;padding:6px">${snapshot.secondaryCount}회 (desc ${snapshot.secondaryInDesc}회)</td></tr>
    ...
    <p style="color:#9CA3AF;font-size:11px;margin-top:20px">매일 KST 10:30 자동 — daeguhoppa-perfect-keyword-watch.mjs</p>
  </div>`;
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [TO],
      subject: `[놀쿨][⚠] 대구호빠 퍼펙트 SEO 회귀 ${issues.length}건`,
      html,
    }),
  });
