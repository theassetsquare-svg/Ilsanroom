/**
 * 이태원클럽 프리즘 단일 키워드 SEO 24h watch — 시즌148.
 * 매일 KST 13:30 — title/desc/body 회귀시만 메일.
 *
 * 측정 키워드:
 *   "이태원클럽" — 단일
 *
 ...
 *  10) 디테일 토큰 (DJ 부스 3미터·멜로딕 하우스·사이키델릭 트랜스·둘째 넷째 토요일·트랜스 마니아) 모두 등장
 *  11) "이태원" + "클럽" 동시 흡수 (검색 분리어 보조)
 *     — Maid(시즌145 1번출구·지하 2층·미군 기지·올드스쿨·밀착) 0/5 겹침: 미군 힙합 R&B
 *     — Savage(시즌146 3번출구·메인 스트리트 한 블록·그래피티 아트·세비지 펀치·금요 자정) 0/5 겹침: 언더그라운드 EDM
 *     — Utopia(시즌147 녹사평 2번출구·해밀턴호텔·레게톤·아프로비트·살사 바차타) 0/5 겹침: 라틴 다문화
 *     — Prism: 음악 마니아 (DJ 부스 3미터·사이키 트랜스·둘째 넷째 토요일) 페르소나
 */
...
const URL = 'https://nolcool.com/clubs/itaewon/itaewonclub-prism/';
const KEYWORD = '이태원클럽';
const DETAIL_TOKENS = ['DJ 부스 3미터', '멜로딕 하우스', '사이키델릭 트랜스', '둘째 넷째 토요일', '트랜스 마니아'];
...
function fetchHtml(url) {
  ...
  const _once = () => new Promise((res) => {
      ...
      https.get(url, { headers: { 'User-Agent': 'NolcoolPrismWatch/1.0' } }, r => {
        const chunks = [];
...
async function main() {
  ...
  const titleSplitHit = title.includes('이태원') && title.includes('클럽');

  if (!titleHasKw) issues.push(`title "${KEYWORD}" 흡수 X`);
  if (!titleSplitHit) issues.push('title 이태원+클럽 분리어 동시 흡수 X');
  ...
  console.log('이태원클럽 프리즘 SEO watch');
  console.log('  URL:', URL);
...
async function sendMail({ issues, snapshot }) {
  ...
  const html = `<div style="font-family:sans-serif;max-width:680px;margin:0 auto;padding:20px">
    <h2 style="color:#DC2626">[⚠ 이태원클럽 프리즘 SEO 회귀] ${issues.length}건</h2>
    ...
    <p style="color:#9CA3AF;font-size:11px;margin-top:20px">매일 KST 13:30 자동 — itaewonclub-prism-keyword-watch.mjs</p>
  </div>`;
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [TO],
      subject: `[놀쿨][⚠] 이태원클럽 프리즘 SEO 회귀 ${issues.length}건`,
      html,
    }),
  });
