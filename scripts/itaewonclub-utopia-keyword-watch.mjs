/**
 * 이태원클럽 유토피아 단일 키워드 SEO 24h watch — 시즌147.
 * 매일 KST 13:25 — title/desc/body 회귀시만 메일.
 *
 * 측정 키워드:
 *   "이태원클럽" — 단일
 *
 ...
 *  10) 디테일 토큰 (녹사평역 2번 출구·해밀턴호텔·레게톤·아프로비트·살사 바차타) 모두 등장
 ...
 *     — Utopia: 라틴 다문화 (레게톤·아프로비트·살사 바차타) 페르소나
 */
...
const URL = 'https://nolcool.com/clubs/itaewon/itaewonclub-utopia/';
const KEYWORD = '이태원클럽';
const DETAIL_TOKENS = ['녹사평역 2번 출구', '해밀턴호텔', '레게톤', '아프로비트', '살사 바차타'];
...
function fetchHtml(url) {
  ...
  const _once = () => new Promise((res) => {
      ...
      https.get(url, { headers: { 'User-Agent': 'NolcoolUtopiaWatch/1.0' } }, r => {
        const chunks = [];
...
async function main() {
  ...
  console.log('이태원클럽 유토피아 SEO watch');
  console.log('  URL:', URL);
...
async function sendMail({ issues, snapshot }) {
  ...
  const html = `<div style="font-family:sans-serif;max-width:680px;margin:0 auto;padding:20px">
    <h2 style="color:#DC2626">[⚠ 이태원클럽 유토피아 SEO 회귀] ${issues.length}건</h2>
    ...
    <p style="color:#9CA3AF;font-size:11px;margin-top:20px">매일 KST 13:25 자동 — itaewonclub-utopia-keyword-watch.mjs</p>
  </div>`;
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [TO],
      subject: `[놀쿨][⚠] 이태원클럽 유토피아 SEO 회귀 ${issues.length}건`,
      html,
    }),
  });
