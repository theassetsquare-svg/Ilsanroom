/**
 * 강남청담클럽 유토피아 DUAL 키워드 SEO 24h watch — 시즌140.
 * 매일 KST 12:50 — title/desc/body 회귀시만 메일.
 *
 * 측정 키워드:
 ...
 *  12) 디테일 토큰 (학동사거리·아이보리 대리석·보컬 트랜스·수요일 레이디스 프로모션·반원형 라운지석) 모두 등장
 *     — 라퓨타·레이스·미로·밤앤밤·사운드·아르떼·아르쥬와 0/35 겹침: 클러빙 입문자 페르소나 + 보컬 트랜스 정점 시그니처
 */
...
async function main() {
  ...
  console.log('강남청담클럽 유토피아 SEO watch');
  console.log('  URL:', URL);
...
async function sendMail({ issues, snapshot }) {
  ...
  const html = `<div style="font-family:sans-serif;max-width:680px;margin:0 auto;padding:20px">
    <h2 style="color:#DC2626">[⚠ 강남청담클럽 유토피아 SEO 회귀] ${issues.length}건</h2>
    ...
    <p style="color:#9CA3AF;font-size:11px;margin-top:20px">매일 KST 12:50 자동 — gangnamclub-utopia-keyword-watch.mjs</p>
  </div>`;
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [TO],
      subject: `[놀쿨][⚠] 강남청담클럽 유토피아 SEO 회귀 ${issues.length}건`,
      html,
    }),
  });
