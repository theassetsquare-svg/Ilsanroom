/**
 * 이태원클럽 세비지 단일 키워드 SEO 24h watch — 시즌146.
 * 매일 KST 13:20 — title/desc/body 회귀시만 메일.
 *
 * 측정 키워드:
 *   "이태원클럽" — 단일
 *
 ...
 *  10) 디테일 토큰 (이태원역 3번 출구·메인 스트리트 한 블록·그래피티 아트·세비지 펀치·금요 자정) 모두 등장
 *  11) "이태원" + "클럽" 동시 흡수 (검색 분리어 보조)
 *     — Maid(시즌145 1번출구·지하 2층·미군 기지·올드스쿨·밀착) 0/5 겹침: 언더그라운드 거친 EDM 페르소나
 */
...
const URL = 'https://nolcool.com/clubs/itaewon/itaewon-savage/';
const KEYWORD = '이태원클럽';
const DETAIL_TOKENS = ['이태원역 3번 출구', '메인 스트리트 한 블록', '그래피티 아트', '세비지 펀치', '금요 자정'];
...
function fetchHtml(url) {
  ...
  const _once = () => new Promise((res) => {
      ...
      https.get(url, { headers: { 'User-Agent': 'NolcoolSavageWatch/1.0' } }, r => {
        const chunks = [];
...
async function main() {
  ...
  console.log('이태원클럽 세비지 SEO watch');
  console.log('  URL:', URL);
...
async function sendMail({ issues, snapshot }) {
  ...
  const html = `<div style="font-family:sans-serif;max-width:680px;margin:0 auto;padding:20px">
    <h2 style="color:#DC2626">[⚠ 이태원클럽 세비지 SEO 회귀] ${issues.length}건</h2>
    ...
    <p style="color:#9CA3AF;font-size:11px;margin-top:20px">매일 KST 13:20 자동 — itaewon-savage-keyword-watch.mjs</p>
  </div>`;
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [TO],
      subject: `[놀쿨][⚠] 이태원클럽 세비지 SEO 회귀 ${issues.length}건`,
      html,
    }),
  });
