...
async function sendEmail(env: Env, to: string, subject: string, html: string) {
  const res = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from: '밤키 <noreply@ilsanroom.pages.dev>',
      to: [to],
      subject,
      html,
    }),
    ...
    switch (action) {
      case 'contact': {
        ...
        if (NOTIFICATION_EMAIL) {
          await sendEmail(context.env, NOTIFICATION_EMAIL, `[밤키] 문의: ${name}`,
            `<h2>새 문의</h2>
        ...
        // 사용자에게 자동 응답
        await sendEmail(context.env, email, '[밤키] 문의가 접수되었습니다',
          `<div style="font-family:sans-serif;max-width:480px;margin:0 auto">
            ...
            <p style="color:#888;font-size:12px">밤키 | ilsanroom.pages.dev</p>
          </div>`
        );
      ...
      case 'venue-inquiry': {
        ...
        if (NOTIFICATION_EMAIL) {
          await sendEmail(context.env, NOTIFICATION_EMAIL, `[밤키] 업소 문의: ${venueSlug}`,
            `<h2>업소 문의</h2>
        ...
        await sendEmail(context.env, email, '[밤키] 업소 문의가 접수되었습니다',
          `<div style="font-family:sans-serif;max-width:480px;margin:0 auto">
      ...
      case 'report': {
        ...
        if (NOTIFICATION_EMAIL) {
          await sendEmail(context.env, NOTIFICATION_EMAIL, `[밤키] 신고 접수: ${postId}`,
            `<h2>게시물 신고</h2>
      ...
      case 'welcome': {
        ...
        await sendEmail(context.env, email, '밤키에 오신 것을 환영합니다! 🎉',
          `<div style="font-family:sans-serif;max-width:480px;margin:0 auto">
            <h2 style="color:#8B5CF6">환영합니다${name ? `, ${name}` : ''}! 🎉</h2>
            <p>밤키 회원이 되신 것을 축하합니다.</p>
